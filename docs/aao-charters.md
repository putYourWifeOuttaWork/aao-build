...
**v3.3 · 5 August 2026 · Two corrections from Matthew, same day. (1) DREAMFORCE IS NEVER A DECISION FACTOR: the clause is struck from v3.2's recorded reasoning; the backburner's reasoning is scale and focus. (2) THE ACCOUNT-GRAIN DELTA IS SMALL: one family out and one in, Buyer Role goes away, Decision Orientation comes in, the rest shared; the byte-level wording recovery stands deferred with the grain. The backburner is unchanged and its standing instruction is: accounts are backburnered, never forgotten. A third note at Ruling 2: call 0's resolution covers the internal side, domain recognition for internal against external, the role map for seller against service, and the resolved record IDs travel in the roster so no downstream stage re-resolves identity. [CODE, same day: v3.1 and v3.2 heads are not in this repo copy; this stamp records v3.3 and the full text lives in the project. See BUILD_JOURNAL for the call 0 gate result.]**
...
**The scaling law, unchanged in substance.** Cost scales with transcript length, never roster size. Call 1 is one bounded read. Call 2's input is pairs plus the candidate list plus a measured window of surrounding turns per pair for context; it is small by construction and splits by pair batch when measured input approaches the MEASURED threshold; call 3 splits per claim batch the same way. No call retries by growing. A quiet call costs three calls; a dense one costs a few more; the count follows how much of consequence was said.
...
**Models. MEASURED.** Calls 1 and 2 on the strong model (call 2 may prove cheap enough to downsize; measured, not assumed); call 3 on the smallest model that passes its re-comparison gate on the new claim shape. Tiers are config; no model name is law.

### §P8.1 · Call 1 · LOCATE AND PAIR

**The job.** One read of the whole normalized artifact. Three outputs, all proposals, all consumed by code.
...
**[v2.6: the model-read legs of this ladder (the Test 2 scope read and Test 3's content match) execute inside call 1 of the pass, per §P8.1; the deterministic tests, the two-key lock, and every rule below are unchanged and stay in Apex.]**
