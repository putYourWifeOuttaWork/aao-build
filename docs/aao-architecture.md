...
**v4.12 · 8 August 2026 · THE COMPOSED NORMALIZER STAMP, AND THE REPAIR THE SCHEMA RIGHTLY REFUSED. `AAO_NormalForm.stamp(rawSha, outputSha)` writes in the Source before-insert; `AAO_Source__c.AAO_Raw_SHA256__c` is new so the ruled form is reachable rather than the fallback quietly becoming the stamp; the prefix says which half answered, `NF1+raw:` the ruled form that can tell two normalizations of one input apart, `NF1+out:` the fallback that cannot collide across outputs but cannot prove shared input. THE NAMED REPAIR OF EXISTING ROWS WAS REFUSED by the standing immutability rule on `AAO_Normalizer_Version__c`, and the refusal is RATIFIED: every stored span was byte-verified against that record, and a provenance field a later process can rewrite is not provenance. The ledger splits, stated rather than hidden: rows before this date keep uncomposed `NF1` permanently; everything after composes; the two 29 July rows stay indistinguishable by stamp, which is exactly the work the explicit freeze list keeps doing. THE LAW LINE, third schema refusal proven right (after `AAO_Raised_At__c` and the claim ledger's insert-only rule), recorded verbatim as doctrine: a law that only permits corrections it can prove safe will sometimes refuse one that is safe, and paying that cost is what makes the law worth having. HANDOFF: the 29 July extraction run goes to a fresh CODE session per the fifteenth stamp; fixture `9e974006` and the corpus list stand frozen; owed from Matthew, carried: the gate 2 three-line coverage grade and the production transcripts as files.**

**v4.11 · 8 August 2026 · THE SPLIT'S SECOND SAVE: B&V RE-ADJUDICATES TO DUAL, AND NONE DOES NOT DISPATCH. Call 0 on the B&V fixture returned DUAL under the one-way halves, both sides byte-located (MCP scoping, pricing and timeline at 20759; adoption and utilization at 36834). GATE ASSERTION 2 IS RE-ADJUDICATED TO DUAL on Matthew's own ruled caveat (expansion seeding on an account call is the dual trigger, not a scope flip) with the v0.3 record concurring; the prior ACCOUNT pass under the retired two-sided question is reclassified as the majority eating the minority in the mirror direction of the Emerson failure. The split has now corrected an under-call on both of its live fixtures. RULED: SIDE-YES WITH DEAL-NONE DOES NOT DISPATCH, the standing silence law reaching dispatch: routed-not-dispatched, journalled with the yes-quote as its reason, symmetrical with the backburnered account grain; NONE treated as honest because expansion talk may precede any deal record; the yes-with-NONE signal carried to the Flags fold as the expansion-detection specimen. Consequence stated: the B&V fixture produces zero establishments this phase and the zero is correct output; its corpus job is the resolver's generalization, performed twice. CODE's stop before spending a pipeline on an open scope premise is the discipline working.**

**v4.10 · 7 August 2026, late · OCCASION IDENTITY AND THE ALIAS ARE LIVE; THE COVERAGE NUMBERS ARE HONEST; THE FREEZE LIST NAMES NF1 ONLY. 383 tests, 382 AAO pass. `AAO_Source__c.AAO_System_Ref__c` is new because the ruled key named something the schema lacked, and the fallback is prefix-honest (`sys:` against `acct:`) so account-plus-occurred can stand in for a key without silently becoming one; the fallback's same-second collision weakness is stated, not discovered, and a real system ref removes it. The alias is read-only by construction, email then name-within-account, never-pick, reaching the coverage derivation and nothing else; the unresolved-unresolved gap stays visibly split. Corrected numbers: Ryan 3 to 2 by dedup, Jefferson 1 to 2 by alias, derived and map agreeing on every row, Jefferson's map moving Brief to Multiple contacts on re-projection with counter, political, and role untouched. RULED on CODE's freeze question: THE FREEZE LIST NAMES THE NF1 ROW ONLY (`9e974006` for 29 July), because the stored form is the fixture form and byte offsets mean nothing against any other bytes; raw rows remain in the org as provenance, listed nowhere. Gate 2 goes to Matthew for grading on the corrected lines; gate 3's pull-normalize-freeze proceeds.**

**v4.9 · 7 August 2026, night · TWO KEYS FOR TWO JOBS: THE OCCASION SPLITS FROM THE HASH; THE ALIAS IS RATIFIED; THE CAVEAT WAS PRESENT AND IGNORED. (1) THE DEDUP DEFECT, from CODE's diagnostic: the 29 July call is seeded twice, raw and NF1, same conversation to the second, two artifact hashes, so Ryan's honest occasion count is two and the map value is accidentally correct, a lucky answer by this build's own standard. The hash exists so one call arriving as three Source rows reads as one occasion, and it fails across two normalizations of one call because it hashes the normalized bytes rather than the conversation. RULED: TWO KEYS FOR TWO JOBS. The artifact hash remains the byte-provenance key for spans, unchanged. OCCASION IDENTITY IS THE CONVERSATION: source-system reference plus occurred time, deterministic, never the normalization's bytes. Coverage counts conversations. Both 29 July artifacts stand (nothing is deleted; NF1 is the stored form); they resolve to one occasion. (2) THE ALIAS PROPOSAL IS RATIFIED as proposed and for the reason proposed: claims key the Participant, so a row merge rewrites what claims point at, a deletion wearing a merge's clothes; aliasing at read time on the resolved Contact leaves both rows readable and makes the derivation correct. The named cost stands recorded: two unresolved rows for one human stay visibly split, never joined by guessing at names. Promotion, not merge, if rows should ever become one. (3) THE CAVEAT DIAGNOSTIC: PRESENT AND IGNORED. `AAO_ResolverCharter.CAVEATS` carries the late-stage-onboarding caveat verbatim, the stamp covers it, and the read paraphrased it back and answered yes anyway: it knew the rule, said the rule, did not apply it. Failure one of the new structure; the count starts; a wording fix has nothing to grip, the courtesy shape's lesson standing. If it fails twice the candidate structure is a caveat gate the yes must pass through, named now so the second failure finds it waiting, not proposed. (4) SEQUENCING, design's ruling on CODE's honest stop: the dedup fix lands BEFORE the corpus freezes, because the 29 July fixture is the double-seeded one and a freeze list would bake the defect into the thing gate 3 measures; pulling and normalizing may proceed in parallel; nothing freezes until occasion identity is live. Coverage grading (gate 2) follows the dedup fix and the alias, in that order, then Matthew grades the corrected derivation lines.**

**v4.8 · 7 August 2026, evening · RUN a22: GATE 1 NOT PASSED ON ITS OWN EVIDENCE RULE, GATE 2 SURFACES THE SPLIT IDENTITY, AND THE RUN ITSELF IS THE STRONGEST YET. Verified from the rows: 44 located (9/12/12/11), 18 upheld one-for-one with claims, regressions 5 to 3, traps 0, q9 HELD as graded, the annual-total criterion PROJECTED to the vendor object for the first time, the full 34-line disposition shipping the whole universe including four STILL MISSED unders and three UNREACHABLE orientation rows. (1) GATE ASSERTION 1 IS NOT PASSED, design's ruling ratifying CODE's own reading: call 0 returned DUAL with both sides byte-located, but the account-side yes quoted the rollout/enablement stretch Matthew ruled in-deal, hedged by the reader's own note, while the enterprise-AI-strategy content at ~34423, which the family reads demonstrably see (deq11 and poq11 quote those bytes), went unnamed. A verdict-only pass is a lucky answer; the assertion is REFINED to name its evidence: the account-side yes must quote one of the two ruled account-grain stretches. DIAGNOSTIC BEFORE STRUCTURE, owed from CODE: does the account-side question's prompt carry the sided ontology's authored caveats, the late-stage-onboarding caveat being the one this read just violated? If absent, adding the ruled caveat text to the declared question completes the ruled design and is not new wording; if present and ignored, this is failure one of the new structure and the count starts. (2) GATE 2's derivation lines carry two findings. THE SPLIT IDENTITY: Jefferson is two participants, the unlinked pre-create rows at 2 occasions and the linked identity at 1, so one human's coverage splits instead of summing and only the linked identity reaches the map. RULED: ONE HUMAN, ONE PARTICIPANT; once the ladder resolves two identities to the same person their occasions sum; CODE proposes the merge or aliasing mechanism before building, and coverage grading waits for it. THE COUNT QUESTION: Ryan derives Multiple contacts from THREE distinct artifact hashes on a fixture corpus that is one frozen artifact; name the three. If harness or test artifacts count toward coverage, the derivation's scoping is wrong and every value is suspect; coverage grading waits for this answer too. Sellers deriving coverage rows (Renee, Wendy, null on map) is recorded as noise, not defect. (3) RATIFIED from the run's own findings: the conjunct now rides homogeneous family batches (a guard reachable only on a rare path is not there); the coverage watermark over-reach narrowed (a computed dimension has no Answer to carry a watermark; the same correction-disables-what-it-corrects shape a third time, caught before shipping); the criteria output ceiling raised after truncation at 7,500. (4) THE JOIN'S NEXT LEVER IS THE CALLER-SIDE SPLIT, priced from the governors: bulkification doubled SOQL headroom (100/100 at 17 to 55/100 at 18, wall at ~32 pairs) and exposed DML co-binding at 81/150, never printed before; no read-side fix moves a DML wall. Queued behind the gate work, ahead of Problems. GATE 3, multi-transcript, begins now on CODE's side: fixture pull, normalization, freezing, per the ninth stamp.**

**v4.7 · 7 August 2026 · THE MAP READ'S LAST CHECK CAUGHT A DEFECT, THE CONJUNCT IS STRUCTURAL, AND THE REGRESSION SET CARRIES THE READ'S GRADES. 383 tests, 382 AAO pass, the named org-resident failure. (1) THE FATEMA CHECK FAILED, AND NOT AS A FORMALITY: `humanEdited` compared two stamps and returned false when ours was null, first contact having been carved out as ours to write; correct while the only way to hold no watermark was to have never met the row, broken the moment a human authored a value on a row we never touched. Matthew's Inner Circle would have been overwritten by the first run to derive her Political Status. FIXED with retraction's own test moved one stage earlier, and the reasoning is law: A NON-BLANK VALUE OUR WATERMARK DOES NOT CLAIM WAS PUT THERE BY SOMEBODY ELSE, because nobody else could have; absence of our watermark plus presence of a value is evidence of a human edit, per dimension, never a place to spend a guard. The deliberate-edit step earns a permanent place in every future read procedure; it found what no green suite had. (2) THE COURTESY CONJUNCT IS STRUCTURAL, the scope split's move again: the target is its own required question, the verdict is arithmetic over the two in Apex, upholding a stance requires naming us as its target and naming the occasion is what refuses it; sentiment only; the reader's reason survives under the refusal as calibration evidence. Call 3 carried zero tests through the change, the second such component in two days; the green-suite hazard stands confirmed as a pattern, not an incident. (3) THE REGRESSION SET IS 39, the map-read grades HELD as assertions on the a21 pairs (Jefferson's two MENTOR quotes, PS2 at offset 7903), and the byte-keying law proved itself on its first exercise: `poq5` names different pairs on a20 and a21, so a ref-keyed assertion would have asserted the wrong establishment. (4) NAMED BOUNDARY, correct and expected: `traps reproduced 1` persists until a fresh run, because the conjunct reaches nothing already verified, the same boundary the never-projected rule had and the reason retirement exists beside it. PROBLEMS IS THE NEXT WORK, design-side charter first; CODE expects the declared-set expansion and nothing else until it lands.**

**v4.6 · 6 August 2026, late night · THE RETIREMENT PATH IS BUILT AND THE FIRST CORRECTION TRAVELLED IT END TO END. 374 tests, 373 AAO pass, the named org-resident failure. Ryan Couture: sentiment answer TRUE to UNVERIFIED, counter 1 to 0, map Supporter to blank with `RETRACTED(Support)` journalled; claim CLM-00000073 retired in place with its reason, still in the ledger, still carrying its quote and verdict. The map has no known-wrong value on it. The trigger exception is one-way and describe-checked, so a field added later is immutable by default rather than by memory. THREE DEFECTS FOUND BY BUILDING IT, each recorded: (1) retirement would have been only a label, because nothing else ever ran the replay after one; it now reconciles in its own transaction over every claim read, because a correction path whose safe response to uncertainty is not-running is not a correction path; (2) `verdictFromClaims` conflated two zeros, no-claims-given against every-claim-retired; zero live claims is now a real answer landing UNVERIFIED, which places nobody, so projection retracts; (3) A RETRACTION IS A WRITE AND IT WAS NOT STAMPING ITS WATERMARK, found by reading the corrected export: `stampProjected` skipped nulls and a retraction's value is null by definition. Left alone, the one write that exists to correct a mistake would have permanently disabled the dimension it corrected, the next pass reading blank-against-Supporter as a human clearance and never writing again. Fixed, and the one stale a21 watermark repaired by a NAMED repair, distinct from the refused backfill: the value known rather than inferred, the stamping half of a write that really happened, and the repair refusing itself if the map is not blank. CARRIED to the Flags and Guidance fold with its specimen: what a seller wants told is not that a field went blank but that we established something and then withdrew it; the note says it, the vendor field cannot. READY FOR THE MAP READ; standing held for it: the occasion-versus-solution conjunct and Jefferson's +3.**

**v4.5 · 6 August 2026, night · THE TRAP-PROJECTION DEFECT CONFIRMED FROM THE ROWS AND FIXED AT THE CLAIM; THE SCOPE SPLIT BUILT; CLAIMS GAIN A RETIRED STATE. 367 tests, 366 AAO pass, the named org-resident failure. (1) THE TRAP PROJECTED, confirmed from the a21 rows with no ambiguity: seq9 upheld by call 3, Ryan Couture's only sentiment claim, counter moved, map wrote Supporter. Addendum 17's never-projected rule was one rule with two halves and only the reporting half was ever built. FIXED AT THE CLAIM, not the write, deliberately: the counter and the verdict replay rebuild from claims, so a projection-side refusal would be undone by the next replay. Verified from the calling runtime against the real rows: 17 upheld in, one trapped by `trap-seq11-courtesy`, 16 surviving. (2) CLAIMS RETIRE, NEVER DELETE, the locked record law reaching the claim ledger: a trapped or otherwise-wrong standing claim is marked retired in place with its reason; the replay excludes retired claims; retraction clears the map value the retired claim had carried. Evidence of the mistake stays readable forever; the a21 Supporter on Ryan's row is the first correction through this path. (3) THE SCOPE SPLIT IS BUILT per Charters v3.4, the `scope` field gone from the schema entirely, verdict derived in Apex, and CODE's three additions are ratified as law of the split: a yes must bring words that byte-locate in the frozen artifact (a non-locating yes routes reads at content nobody said); a missing side THROWS rather than defaulting to no (absence read as no would reproduce the under-call being repaired); and NEITHER is sayable, dispatching nothing, journalled. (4) FINDING, its own line: `AAO_ResolverCharter` carried ZERO tests through two structural changes, so a green suite said nothing about the component that had failed twice; twelve tests exist now, made possible by the split (the verdict used to be a string the model wrote). The standing lesson joins the hazards: A COMPONENT'S SUITE MUST GROW WHEN THE COMPONENT CHANGES SHAPE, and a suite that cannot see a component is green about nothing. (5) THE JOIN IS BULKIFIED with the load-bearing exclusion reasoned and ratified: three of four reads are batch-invariant and cached; `readExisting` stays live because a candidate may read an answer written earlier in the same loop; the writer is not rewritten to lists because a second copy of human precedence, merge, and reinforcement is exactly what going through `AAO_Commit` exists to prevent. Bulkification is priced by the next full run; governor consumption prints per stage with a marker past 80%. (6) HELD, unchanged: the occasion-versus-solution conjunct and Jefferson's +3, both Matthew's at the map read.**

**v4.4 · 6 August 2026, afternoon · CODE'S ANSWERING FOLD ABSORBED, THE PROPOSALS RATIFIED, AND THE EMERSON SCOPE RULED BY SPLIT. (1) CLOSED BY CODE, each verified from the runtime: `ALTF__Contact__c` is `nillable = false`, the narrower shadow reading now stands on read fact; the failing test is `ConvertToOpportunityTest.testgetOppCreationDetails`, org-resident, four-fold evidenced, reported henceforth as 350 AAO pass beside one org-resident failure; GATE ASSERTION 2 PASSED, B&V 24 June reads ACCOUNT on its first run; the clobber is fixed twice over, a write-time guard plus the order-independent replay; the regression universe defect found and fixed (`checked++` sat after the unreachable and correct-refusal branches, the seed was larger than reported, and five assertions fell through every branch while counted in the denominator; the universe now reconciles and the full per-assertion disposition ships every run); the sweep split is built per the ruling. (2) DEFECT, CODE's own naming, fix before the rerun: the B&V verdict's `scopeBecause` returned the literal string `placeholder`; a required reason field satisfied by a non-reason is a field that looks answered, the defaulted-coverage shape again; a verdict without a real reason fails loudly, never fills. (3) RATIFIED, design's under the delegated mode: COVERAGE derives from distinct artifact hashes, never rows; `In-depth` is deliberately underivable because meaningful and high quality are not counts; ZERO OCCASIONS WRITES NOTHING, an assertion from absence being the silence law's own case. RETRACTION IS WHAT RE-RUN DOES when the derivation empties: projection writes the current derivation including null, only where the watermark proves the value ours and unmodified, the note carrying what the vendor field cannot; no flag on retraction today, journalled always, revisited at the Flags and Guidance fold. (4) THE EMERSON SCOPE ADJUDICATION, Matthew's ruling, SPLIT THE KEY: the rollout, training, enablement, and CSM/licensing rows re-read as in-deal scoping of future services, his own authored caveat governing (late-stage onboarding discussion inside a still-open deal reads account-shaped and is not); ENTERPRISE AI STRATEGY and ACCOUNT-LEVEL BUSINESS-CASE CONTROL stay account-grain, facts about the company rather than the purchase. Gate assertion 1 therefore STANDS at DUAL on narrower grounds, two stretches instead of five, and a17's OPPORTUNITY remains an under-call. First occurrence of the shape; per the failed-twice law no wording moves; the rerun reports its verdict with a real `scopeBecause` and a second miss earns structure. The adjudication sheet's §8 is marked with the split in place.**

**v4.3 · 6 August 2026, midday · RUN em0806-a20, THE FIRST FULLY VERIFIED RUN, and the loop caught the writer's next defect in its rows. (1) THE RUN: all four families answered (Sentiment 9, Political 10, Buyer Role 9, Criteria 5; 33 located, 11 upheld, 22 refused, ledger 33/33/33); the blank-retry guard live and reported a non-fire as a zero, never silence; SUPPORT REACHED THE MAP FOR THE FIRST TIME, Jefferson +2 Supporter and Neeraja +1, §P8.4's worked specimen on live data; `emerson-q13` reported CHANGED undiagnosed, the third state doing its job. Row export and timings shipped per the inbox; design's verification ran on the rows themselves: sha256 match, ALL 33 QUOTES EXACT AT THEIR STATED OFFSETS, every quote unique in the artifact, all 11 upheld pairs carrying claims and no refused pair carrying one. Zero fabrication is now machine-verified per run, not asserted. (2) DEFECT, found in the rows: PARTIAL CLOBBERS FULL. An upheld pair with Partial coverage writes an UNVERIFIED accrual claim, and the join lets it displace a TRUE the same run established from Full evidence: buq4 downgraded Jefferson's Evaluator answer TRUE to UNVERIFIED after buq1 established it Full and Upheld; poq5 did the same to his PS2, and that demotion then flowed through the political ceiling and changed his map placement. The law: partial accrues, places nobody, and DISPLACES NOTHING; an answer's verdict is monotone over evidence strength, never last-write-wins. Fix the join and replay answers from claims, the counter's own rebuild pattern. (3) DEFECT, its surface: ANSWER AND PROJECTION DIVERGE. Two answers stand UNVERIFIED while the map carries their old projected values under stale or same-run watermarks. When an answer changes state, projection re-runs or retracts; which, under the human-override law, is CODE's to propose before building. (4) RULING, design's under the delegated mode: THE SWEEP SPLITS CALLER-SIDE, one family read per transaction, the call 3 pattern. The a20 timings bind at the transaction, not the callout: worst single read 15,580 ms, 13% of the ceiling, but four reads in one transaction is 57,054 ms, 48%, and twice this transcript or a fifth family breaches it. Split, the binding number becomes the worst single read and the account grain's future families cost no ceiling anxiety. (5) RATIFIED: `AAO_Criterion__c` IS OURS and the vendor row is its projection; CODE's reading stands on LAW #1 and claims-land-on-ours-first; Architecture v2.1's fifth-subject-with-no-new-entity is superseded and marked at this head. The three criteria contracts join by byte-range overlap, the regression set's own mechanism. (6) REPORT DISCIPLINE, still short: the regression report accounts 16 held plus 8 regressed of 27, three dispositions missing even counting q13's CHANGED line, and the full 27-line disposition list is owed for the SECOND consecutive run; 11 answers reported against 9 shipped; "2 populated, 5 dimension values" still lacks denominators. Standing unanswered from v4.2: the B&V call 0 run, the Emerson DUAL adjudication, the failing test's name, the `ALTF__Contact__c` describe.**

**v4.2 · 6 August 2026, morning · THE FIRST VERIFICATION FOLD, five-lens agent audit of the a17/a19 record, and it found what everyone missed. (1) CALL 0 FAILED ITS FIRST GATE ASSERTION AND THE RECORD CALLED IT SUCCESS: the gate asserts Emerson reads DUAL (the five account-grain rows are attached to that call); a17 returned OPPORTUNITY, the collapse-to-majority-scope its own riskiest-assumption note predicts; assertion 2 (B&V 24 June reads ACCOUNT) has never been run; "call 0 stands verified" in this document and the Board is retracted, and the v4.1 celebration is marked wrong in place. Owed from CODE: the B&V run, and a report of what the scope read saw on Emerson's account-grain stretches before any wording moves. (2) THE REGRESSION REPORT DOES NOT ACCOUNT FOR ITS UNIVERSE: 14 held plus 9 regressed is 23 of 27, "2 graded misses recovered" has no source population (the s1 ruling was zero misses found), and zero located criteria pairs cannot account for 7 regressions when only 6 criteria assertions exist. Every future regression report lists all 27 dispositions, one line each; a harness whose categories do not sum to its universe can silently drop assertions. (3) "336 TESTS, 100% PASS, ONE PRE-EXISTING UNRELATED FAILURE" is self-contradictory and the unnamed failing test is the green-but-unrelated pattern that hid derive(); the failing test is named once with evidence of its irrelevance or it stops being waved through. (4) UNTRIED SURFACES, named so they stop reading as verified: the sentiment fix has produced zero landed values in any run; the toggle-OFF leg, shadow create, and promotion path have never executed; the name parser's refusal branch is unexercised; the `ALTF__Contact__c` requiredness underlying the ratified narrower reading has never been checked from the runtime. (5) The sentiment cause and the either-way clause in v4.1 are marked wrong in place below. Byte verification came back clean: every sampled graded quote locates verbatim in the frozen artifact. The verification harness reruns on every iteration; its findings land in this document and the inbox, not in chat.**

**v4.1 · 5 August 2026, night · THE FIRST FULL RUN TO THE MAP, `em0617-a17`, and its rulings. (1) CALL 0'S FIRST LIVE RESOLUTION: projected to the Insights 500 deal, the deal open when the words were spoken, decided on CONTENT; Test 1 reported NOT AVAILABLE (no role map, the file-dump position), and the occurred window reported unanswerable from seed data rather than producing a number. The case the resolver exists for, arriving in the one org where the cheap mechanism was absent. [v4.2: WRONG AS RECORDED, caught by the verification fold. The standing gate asserts Emerson reads DUAL, because Matthew's five account-grain rows are attached to this exact call; OPPORTUNITY is the collapse-to-majority-scope the riskiest-assumption probe was built to catch, and it fired on the first live run while every document including this head recorded it as success. Gate assertion 1 stands FAILED; assertion 2 (B&V 24 June reads ACCOUNT) has never been run. Call 0 is not verified; the v4.2 head carries the adjudication.] (2) THE MAP IS LIVE AND THIN, four named causes: Neeraja Chimata and Ryan Couture carry Evaluator, written by the run; Jefferson Vargas derived (Evaluator, Outside Political Structure) and unreachable, the create leg verified UNWIRED, no toggle existing to read, no gate holding him; sentiment zero because the read emitted an illegal Partial and accrued UNVERIFIED [v4.2: the cause is corrected by CODE's own a19 report. The read never emitted a Partial and the schema already could not express one; the defect was the join reading null coverage as not-Full. This clause and Addendum 19's premise are wrong text, marked here.]; criteria zero located against six graded True on the same artifact, the serious recall finding, seven of nine regressions, the regression report BROKEN at 14 of 27, `emerson-q9` absent 3 of 5. (3) RULINGS, Matthew's, 5 August, Addendum 19: sentiment coverage is WHOLE BY CONSTRUCTION, the schema unable to express Partial; criteria are INSTRUMENTED BEFORE ANY FIX, two repeat sweeps on the frozen artifact with per-family counts and composed stamps against s1, nothing moves until measured; THE CREATE LEG BUILDS NOW, ladder then create then map row and ContactMapDetail either way, failure legible, [v4.2: "either way" across Contact-and-shadow is wrong text; `ALTF__Contact__c` is required on the vendor map row per the v3.0 ruling text, so shadow persons never reach the vendor map and the shadow surface is ours. CODE's narrower reading is RATIFIED. The requiredness itself is carried from ruling text, never yet verified from the runtime; one describe call is owed.] with the requirement that Contact creation never writes the mangled roster string as a name, the deterministic parse applying and the source string kept per the stored-form law; THE TOGGLE EXISTS AND DEFAULTS ON, org-overridable seed-metadata shape, creation allowed by default, Matthew's ruling with design's OFF recommendation recorded; `AAO_Shadow_Person__c` BUILDS NOW as a real object, key, promotion path, AnswerKey wiring, pulled forward from Wave 2. (4) Correction, CODE's own, recorded: the run report described Jefferson's `unresolved` approvingly as the new action doing its job; it reports honestly, and the ruling it reports against was not satisfied.**

**v4.0 · 5 August 2026, night · THE JOIN IS BUILT AND THE RUN IS UNBLOCKED; one placement question surfaces and stays open. From CODE's build, 319 tests, one pre-existing unrelated failure. (1) The three pieces stand: `AAO_PairCommit` runs through `AAO_Commit`, never around it, one copy of human precedence, merge, the speaker gate and reinforcement; `derive()` is re-aimed at `people-p8-v1` through `AAO_P8Codes`, a literal map of seventeen placements, no string surgery; the counter writes `AAO_Support_Counter__c`, one move per establishment, net per call, clamp and voiced-state reach, and `rebuild` replays from claims in evidence-occurred order so drift is unexpressible rather than merely forbidden. Political reads TRUE only: the weak sides are their own contracts, so the guard against inferring a weak side is retired, not relaxed, because what it guarded against can no longer be expressed. An unrecognised code is counted and named on its own line: a zero because nothing was establishable and a zero because the writer recognised nothing are different facts. (2) Four defects surfaced by the build, recorded: `Subject_Person` had refused every time it ever ran, Commit passing a null subject and the gate honestly refusing a check it cannot run, so every sentiment establishment downgraded to UNVERIFIED and the counter would have read zero for everyone; beneath it, the gate compared two resolution outcomes, so a person with no Contact could never satisfy "they said it themselves"; it compares the roster key now, the Participant ruling reaching the gates. And the join re-read committed pairs, double-writing `Reinforced` claims; fixed with the evidence watermark at the pair grain, the claim field on the pair ledger. (3) BUYER ROLE COLLISIONS, CODE's assumption, standing until Matthew rules: the field is single-select and the pass can establish two; the five values are an unordered set, so the writer never picks, says so in the note, and counts the collisions; an ordering, if Matthew rules one, replaces this method; the Emerson run gives the number. (4) OPEN, a placement question, not ordinary work: THE CRITERIA WRITE. The answer key is subject plus contract and the subject is the person, so a person naming three criteria produces one answer per contract; the path needs answers subjected to the criterion, the fifth subject type, unwired. Criteria derivation and counting are built; the `ALTF__Decision_Criteria__c` write is not, and this run carries criteria as answers with receipts only. Likewise the declaration jump has its mechanism and no feed, call 1 emitting no declaration marker; charter-side, small, open. The map read sheet is corrected to v0.3 for both.**

**v3.9 · 5 August 2026 · A correction against the v3.8 head, from CODE's build report, the capability law collecting its fee. v3.8 item 3 recorded projection's step three as "exists and is proven"; marked wrong in place below. `AAO_Project.derive()` was proven only for the retired EBV path. Against the §P8 contract set it fails SILENTLY: `tokenOf` splits on a `#` the seventeen P8 codes do not carry, every answer is skipped, `derive()` returns empty, and projection writes nothing while reporting success; and the Support derivation inside it is the retired nine-question quiz tree that §P8.4's counter superseded. The claim was believed because it was proven for a retired shape and never tried against the live one: A CAPABILITY CLAIM IS UNVERIFIED UNTIL TRIED FROM THE CALLING RUNTIME, INCLUDING CLAIMS ABOUT OUR OWN CODE. The join is three pieces, all ordinary work, none needing a ruling: pairs to Claims to Answers with the Contact-or-shadow create at projection; `derive()` re-aimed at `people-p8-v1`, code parsing, the Political ceiling over the one-way halves, Buyer Role, criteria; and the counter arithmetic writing the ruled field. The guard that travels with the join, the existing law's fourth stage: the stage arithmetic extends through projection, answers in and writes out counted and reported per dimension, so an empty derivation is expressible only as a reported zero, never as success. [v3.9, built same day, 291 tests: `AAO_Project.Outcome` prints answers read and dimension values written, per dimension, and `derivedNothing()` alarms ONLY on answers-in-with-no-values-out; zero answers in is silence, not a silent failure. Printed counter rather than only a test, CODE's reasoning recorded: a test asserts what somebody thought to assert; a counter is read by whoever reads the run, and this defect survived a green suite because nobody had thought to assert it.] The run holds until projection can write, so that Matthew's map read grades the writer's output and never a silent blank. Recorded with the fold: the ruled fields landed as schema (`AAO_Claim__c.AAO_Participant__c` the key, `AAO_Subject_Contact__c` kept for pre-ruling rows and the resolved Contact where one exists; `AAO_Answer__c.AAO_Support_Counter__c` Number(2,0), the standing value only, rebuildable from claims in evidence-occurred order so a drift between field and ledger is a bug, never a tie-break), FLS granted on both, 289 tests passing; the seller-side fix and call 0 stand verified; the sweep and the regression report are ready the moment projection can write.**

**v3.8 · 5 August 2026 · The writer-join placements, ruled at the projection run's unblocking; Charters v3.3 and Addendum 18 carry the day's context. (1) CLAIMS KEY THE PARTICIPANT: `AAO_Claim__c` gains a lookup to `AAO_Participant__c`, our source-agnostic person ledger, and the Contact key retires. A Contact is a resolution outcome of a person, never their identity, and the native ECI participant record is one input to ours, never the thing itself. Identity resolves through the ladder (name, email, the existing gates); an unresolved person holds at shadow stage; at projection the writer's mandatory create leg makes what the map needs, the Contact where the toggle is on, the shadow where it is off, and the ContactMapDetail row either way. Jefferson Vargas, identified but unlinked, goes through the create path on the first projection run. (2) THE COUNTER'S FIELD HOME, placed by design under Matthew's delegation: the standing value lives as a field on the Support Answer row, per person per scope. Answers are already the per-dimension state rows the map projects from, and the citations law already lives there, so every counter move derives from cited claims at reconciliation, clamp and ceiling in code, and projection writes the vendor's Support Status from the Answer. No new object; a counter on Participant would be per person global, wrong the day the account grain returns, because stance is per scope. Model & Flow records the field at its bump. (3) Recorded from CODE's report: nothing joined `AAO_Pair__c` to claims or answers before this fold (`AAO_Answer__c` was written only from the retired EBV path); steps one and two of the join are the pass's missing spine and are ordinary work under these placements; step three, `AAO_Project`'s query-then-branch with the mandatory create leg and per-dimension watermark, exists and is proven. [v3.9: WRONG, marked in place. Proven only for the retired EBV path; against the §P8 contract set it fails silently. The v3.9 head carries the correction.] (4) The internal-domain list is the right value in the wrong home: it lives in the demo static resource and its lawful home is the org-overridable seed-metadata shape, the role-map pattern; owed at packaging, not blocking.**

**v3.7 · 5 August 2026 · The sweep-and-grading fold, carried at the head per this document's convention, superseding the body and earlier heads where named. (1) THE FAMILY SWEEP IS CALL 1'S SHAPE (Matthew, 5 August), superseding the v3.6 head's item 1 "whole artifact once": call 1 executes once per declared family — four bounded reads of the whole artifact, each carrying exactly one family's question set and nothing else. The cost law's placement here: crossing by PERSON is forbidden because it scales with the roster without bound; crossing by FAMILY is permitted because it is bounded at four or five and scales with nothing — not roster, not length, not density — and the code asserts the call count is never a function of participant count. Charters v3.0 §P8.0/§P8.1 carry the charter text; the measured reasoning (the scarce resource is ATTENTION, not prompt size: sentiment 2 → 11 pairs under one unchanged contract; stable establishments tripled; multi-establishment dissolved structurally) is recorded there and on the Board. (2) CALL 3 IS THE ONLY STAGE WHOSE COST IS LINEAR IN HARVEST SIZE AGAINST A HARD PLATFORM CEILING: verification runs one bounded callout per claim batch under the 120-second cumulative transaction law (v3.3 item 1); forty claims at one per callout is 172 s cumulative and exceeds it; three caller-driven batches of fourteen finish inside it. Fine today, FIRST TO BREAK on a denser transcript; the split is caller-driven, never a retry-by-growing. (3) THE TWO-HALVES PROVENANCE LAW lands here: A VERSION STRING MUST COVER EVERY INPUT THAT CAN CHANGE THE MODEL'S INSTRUCTIONS, or it is decoration. Call 1's prompt is two halves — charter prose plus declared contract text — and the stamp composes both (the `locate-2.1.0+d314a73c` shape); under the sweep it composes PER FAMILY READ, four stamps per pass. Provenance of the law: the first Emerson run carried v2.9 prompt text under an unmoved version string, a break caught at grading and closed by the composed stamp. (4) THE REGRESSION SET'S PLACEMENT: a standing run artifact beside the fixtures, executed against the frozen artifacts on every run. Every establishment Matthew has graded PASS is a standing assertion on every run; every graded trap must not appear; CORRECT REFUSALS ARE NEITHER, because asserting one asserts that call 1 must keep making a mistake call 3 can catch. Keyed on contract plus byte-range intersection against the frozen artifact, never the stored string; an undeclared family reports UNREACHABLE on its own line and can never inflate a recall failure. Seeded 5 August from the sweep grading: 18 standing assertions, one trap (courtesy upheld as SUPPORTER), and q9's flickering PASS as the first expected report; the seed travelled to CODE as Addendum 15. (5) Recorded with the fold: the sweep s1 grading passed the gate on the opportunity grain (16 of 17 upheld True, 21 of 23 refusals correct, zero fabrication; the record is the adjudication sheet v0.6 §9), and the run receipt's load RISES under the sweep — four reads per pass make a partially completed pass expressible, and the receipt is what records which family reads ran; the object stays owed by CODE.**

**v3.6 · 4 August 2026, afternoon · The simplification fold, carried at the head, superseding the v3.5 head where named; Charters v2.7 §P8 is the authoritative pass text and this head is its placement. (1) THE PASS IS PAIRS: call 1 (strong model, whole artifact once) resolves deal, account, and candidate people, and emits question-answer pairs, verbatim words plus voiced meaning, no person attached; call 2 (small by construction) attaches each pair's person from the closed candidate list and nothing else; call 3 verifies blind, unchanged. The v3.5 head's item 1 placement stands with call 2's job narrowed to identification. **[v3.7: item 1's "whole artifact once" is superseded by the family sweep — see the v3.7 head.]** (2) NO ABSTENTION ROWS, superseding the v3.5 head's item 4 whole: the v2.6 abstention vocabulary is retired unbuilt; nothing writes silence at any grain; auditability is the run receipt (one row per Source per pass) plus one-for-one-for-one arithmetic between stage row sets, throwing on mismatch per the parse law. The Null / UNVERIFIED / abstention table in the body survives with abstention's row meaning narrowed to the decision log only. (3) THE PAIR LEDGER supersedes the v3.5 head's item 3 anchor ledger: every pair persists twice, initial unowned from call 1 and linked identified from call 2, every stage measurable in isolation; the initial row's byte-located verbatim string is the anchor; entity ruling owed to Model & Flow, CODE proposes the mechanism. (4) TRUE-OR-NOTHING replaces the pass's verdict vocabulary: every question points one way, negative states establish from their own affirmative words, the two-sided Political questions split into one-way halves, and no FALSE or inversion exists anywhere in the pass. (5) THE SUPPORT QUIZ IS RETIRED FROM EXTRACTION and the sentiment counter re-promoted (Charters §P8.4): five states read directly, one counter step per call toward the voiced state and never past it, explicit declarations jump with the sequence flagged, mixed evidence nets to no move with all quotes journalled; Political, Buyer Role, criteria, insights, Process, assessment keep their ontologies; Coverage stays computed. (6) THE OCCURRED-TIME CANDIDATE WINDOW is law for resolution: candidate opportunities are open plus recently closed AS OF THE EVIDENCE-OCCURRED CLOCK, never the run date; the 17 June Emerson fixture is the standing specimen (its only open-at-call-time deal has since closed won while a renewal opened, so a run-date window resolves the call to a deal that did not exist when the words were spoken). (7) THE TEST FIXTURE AND HARNESS: VideoCall 6qrV4000000J5K9IAK, read from production 4 August (41 minutes, Teams, diarized, five participants, two external participants with null RelatedPersonId including Jefferson Vargas who has no Contact at all); the harness satellite `aao-pass-test-harness-v1_0.md` carries the stage-by-stage test suite, the normalization intake matrix, and the CSV review format. (8) Correction carried forward from v3.5 item 8 unchanged: the report-unit fix is queued, not shipped.**

**v3.5 · 4 August 2026 · The rebuild scaffold, carried at the head, owed to the body at its next bump. The charter text the v3.4 head awaited is WRITTEN (Charters v2.6, §P8), and this fold places the rebuild. (1) PLACEMENT OF THE THREE CALLS: the pass is three model-call classes per Source artifact, each a short synchronous callout under the one-bounded-callout-per-transaction law: LOCATE (strong model, whole artifact once, emits resolution proposals and anchors, absorbs the resolver's model-read legs, Test 2 and Test 3's content match, with the deterministic ladder and two-key lock unchanged in Apex), ATTRIBUTE-AND-CLAIM (strong model, anchored excerpts only, one execution per contract group), VERIFY (small model, per claim batch, blindness unchanged). The compute table's Propose and Re-check rows are superseded accordingly at the body's next bump. (2) THE SPLIT MECHANICS, sharpening the v3.4 scaling law: call 2 splits by CONTRACT GROUP, never by excerpt, so each contract is asked exactly once across all splits; the trigger is measured input against a MEASURED threshold below the ceiling's practical budget; the floor is one contract family with its excerpts; no call retries by growing; call 3 splits per claim batch. Numbers live in BUILD_JOURNAL, never here. (3) THE ANCHOR LEDGER enters the inventory as a run artifact on core: immutable, byte-verified at write, keyed to Source and run, read by the recall gate, the abstention derivation, and audit; whether it is a field on Run, a child object, or a sidecar is CODE's proposal first, per the inbox, and its entity ruling is owed to Model & Flow. (4) THE ABSTENTION VOCABULARY is re-ruled at §P8.0: `nobody_said` retired as writable, `not_surfaced` (Apex over the anchor ledger, authority call 1's measured recall) and `not_in_excerpts` (call 2, excerpt-scope only) enter, `not_returned` and the parse law unchanged; transcript-wide silence is derived, never stored; the enum change is owed to Model & Flow's field tables. (5) SUPERSEDED IN THE BODY, marked here until the body bump: the v2.1 sentence that criteria are emitted by the insight charter — decision criteria join People (Charters v2.5 ruling, contracts at §P8.4), and the arbitration argument that motivated the old placement survives structurally because call 2 is now the one reader holding both taxonomies. The criteria projection mechanics in the body (three DML, Name stamped with the record's own Id, Subject as display text, ratification bypass, never-flags) are unchanged and §P8.4 builds on them; DC-F's typing axis is ruled provenance by Matthew this day, grounded in the research satellite. (6) S1 (Support Q1) is RETIRED from every declared claim set (§P8.5); the ontology keeps it byte-exact, not-declared. (7) THE RECALL GATE is the pass's first staged-truth run: strip-and-restore on the 24 June fixture against Matthew's grades plus the canonical criterion, re-run on any call 1 charter change and on any context-window change; it gates trusting call 2 and it is the first measurement of the discovery-recall assumption the envelope section names as the design's riskiest. (8) CORRECTION, from CODE's 4 August report: the v3.4 head's item recording the report-unit fix as done was wrong — commit `9a0720a` changed BUILD_JOURNAL only and the unit-law fix in `AAO_EBV.report()` is queued, not shipped; the unit law stands as law and the Board carries the correction at its rewrite.**

**v3.4 · 4 August 2026 · The three-call fold, at the head, owed to the body at its next bump. (1) THE PASS SHAPE: the three-call pass (locate, attribute and claim, verify) supersedes the one-read extract; Charters v2.5 carries the ruling, and the charter text lands there after the decision-criteria research. Placement here is the scaling law: COST SCALES WITH TRANSCRIPT LENGTH, NEVER ROSTER SIZE (Matthew's ruling, reasoning at Charters v2.5); calls multiply dynamically on measured group cost rather than any one call growing toward the ceiling, so the 120-second law is satisfied by construction because no single call carries the whole job. (2) THE DECLARED-SET LAW RE-SCOPES to the claim path: call 2's contracts are exactly the declared charters'; call 1 reads wide by ruling; the violation remains handing the model an undeclared claim question. (3) THE UNIT LAW, from a measured defect: a run report printed item-unit and pair-unit counters adjacent with no label (16 bound items, 9 rejected items, 6 claim pairs) and the adjudication sheet computed a phantom missing claim from them; every telemetry counter names its unit at the point of print, and counts in different units never sit adjacent unlabeled. (4) RECORDED OPEN, blocking any second pass on the B&V deal: whether the manual projection run wrote the compare-and-swap watermark pair (AAO_Projected_Value__c, AAO_Projected_Modstamp__c) beside the map write; the modstamp cannot be reconstructed later, and if it is missing the next pass reads our own write as a human edit; CODE owes the query. (5) Decision criteria as a projection target (ruled v2.1) now has its charter home named: People, relationship-map only; the fifth Claim subject type is unchanged.**

**v3.3 · 3 August 2026 · The platform-ceiling fold, carried at the head, owed to the body at its next bump. Three laws land. (1) THE CALLOUT CEILING, measured and primary-doc-verified (Apex Developer Guide, Execution Governors and Limits, Summer '26): the maximum cumulative timeout for ALL callouts in one Apex transaction is 120 seconds, identical synchronous and asynchronous, unraiseable by Support, and — unlike SOQL/DML — SHARED across certified managed-package namespaces like CPU and heap. Callout wait time is not CPU time; Batch Apex resets per-transaction limits per execute. Consequence: the wait never lives in Apex — one bounded callout per transaction, short calls as the design target. Three timeouts at the ceiling plus measurement 71 (16 propositions, 87,805 ms, 18,696 in / 7,339 out against 53 propositions at 18,696 in / 7,532 out) established that output is near-constant against proposition count and the artifact dominates input, which retired the per-person pass shape (Charters §P7.3 carries the successor). (2) THE MODEL PATH, for posterity: the Einstein Trust Layer / Models API is and has always been the architecture's model path; the direct Anthropic Named Credential is a dev-org stand-in because flex credits cannot run in the dev org. The Models API is synchronous-only and under the same 120-second ceiling; provider batch endpoints (Anthropic Message Batches, GA Dec 2024, 50% price, ~1-hour typical / 24-hour cap) bypass the ceiling entirely but bypass the Trust Layer with it — recorded as incompatible with trust mode today and PARKED, never sold. BYOLLM is Model Builder behind the same gateway, so the trust posture and the any-model plan are one plan. (3) THE TWO-LANE NOTE reduces accordingly: the universal lane is short synchronous calls in per-transaction units; a bulk lane exists only if the gateway ever ships async. Hazards recorded: the local-fix hazard (Charters v2.4 carries the text); rulings travel to CODE as one explicit line at the top of a post, never a bracket, never bundled behind one word — two decisions were lost in one day to decision-slots in traveling documents.**
...
The four keys — opportunity, account, external person, internal person — **do not include dimension, proposition, card or attribute** — so a journal row as previously specified cannot identify *which claim* it belongs to, and **replay cannot reconstruct the mirror.** That is the invariant this separation exists to make testable, and it was untestable.
...
> **Completeness was removed as a flag type in v1.5, and that ruling stands.** It is now **guidance**, which is not a flag at all. **v2.2 adds yellow back for a different reason and this is not a reversal** — two different things were living under one word and only one of them was a flag. Completeness is a non-gating proposition still unverified past its escalation point, and it belongs to guidance. **Ratification is a write that passed every check and is waiting for a human to permit it.** Only the second is a flag, and the word goes to it. *Stated at length so the correction is legible to anyone reading v2.1 beside v2.2.*
...
**[v3.5: the Propose and Re-check rows below are superseded by the three-call placement carried at the v3.5 head — LOCATE, ATTRIBUTE-AND-CLAIM (splitting by contract group), VERIFY (splitting by claim batch), all Core → Trust Layer, all short synchronous calls. The table is rewritten at the body's next bump; every non-model row stands.]** **[v3.7: and LOCATE is the family sweep — four bounded reads per pass, one per declared family, per the v3.7 head.]**
...
> **[v3.5: the emission-placement sentence above is SUPERSEDED — decision criteria join People (Charters v2.5 ruling; contracts at Charters §P8.4). The arbitration argument survives structurally: under the three-call pass, call 2 is the one reader holding both taxonomies, so the choice between goal and criterion is still made in one place and still auditable. The seed non-establishment rule above stands and binds call 2.]**
