...
---
## §P8 · The pass charters · charters v2.7 · the three calls, simplified
...
**The counter, re-promoted from fallback to Support's mechanism.** One integer per person, minus three to plus three, clamped, with the rungs at their ruled positions: +3 Mentor, +1 and +2 Supporter, 0 Neutral, −1 and −2 Non-supporter, −3 Enemy. Per call, per person: the identified, verified sentiment reads for that person net into one movement, and the counter takes **one step toward the state the evidence voiced, never past it.** Warm words carry Jane to +1; three merely-warm calls leave her at +2, still Supporter; +3 requires mentor-grade words, her actually guiding and advocating inside. Evidence caps its own reach, so warmth alone never inflates to Mentor. Mixed evidence in one call nets to no movement, and every quote still journals as a claim, so the next pass inherits the full picture. **The jump rule:** when the words explicitly declare a state outright ("I'm absolutely not buying your thing," "I'll be selling this internally for you"), not strong feeling but the thing said, the counter jumps to the declared state's position, and a contention flag carries the whole sequence with quotes when the jump reverses standing polarity. Replay over claims in evidence-occurred order reconstructs the counter exactly; determinism holds because the counter is a clamped walk over receipts.
...
### §P8.5 · The decision-criteria contracts · unchanged from v2.6 in substance
