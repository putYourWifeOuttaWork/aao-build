...
**v3.5 · 8 August 2026 · THE COURTESY CONJUNCT IS LAW, folded from the map read. Matthew confirmed it at his read of the live map, 7 August (adjudication sheet §10), and it is built structurally per Architecture v4.7: a sentiment proposition's stance must be voiced toward us or our solution, never toward the meeting or the occasion; the target is its own required question; the verdict is arithmetic over the two, derived in Apex, riding homogeneous family batches so the guard exists on every path. Upholding a stance requires naming us as its target, and naming the occasion is what refuses it. The v3.4 head's held-for-veto clause is marked superseded in place. Also corrected at this stamp: the §P8 section heading below is re-stamped to name this document's current version rather than the version that first wrote it; the section's bracketed amendments were always properly versioned.**

**v3.4 · 6 August 2026 · THE SCOPE READ SPLITS INTO ONE-WAY HALVES, earned by the failed-twice law. Call 0's scope question was two-sided (deal, account, or both) and it collapsed to the majority scope twice on the same fixture: a17 returned OPPORTUNITY against the gate's DUAL, and a21, with a real reason field, correctly re-read the services scoping under Matthew's split ruling yet still failed to surface the two account-grain stretches (enterprise AI strategy, account-level business-case control). Second miss on the same stretches; structure owed and applied: the standing two-sided law (two-sided questions split into one-way halves) reaches call 0. THE SCOPE READ IS NOW TWO ONE-WAY QUESTIONS, each answered independently with quoted evidence: does this call carry opportunity content, and does this call carry account content. DUAL is both answered yes, never a third option competing with a majority. The Part II call 0 section is amended in place below. Recorded with it: courtesy-as-stance has also now failed twice (seq11 graded as the sixth trap at s1; reproduced AND upheld by call 3 at a21), and design's proposed structure, held for Matthew's veto at his map read, is the OCCASION-VERSUS-SOLUTION target conjunct [v3.5: the veto never came and the ruling did: Matthew CONFIRMED the conjunct at his map read, 7 August (adjudication sheet §10), and CODE built it structurally per Architecture v4.7, the target its own required question with the verdict derived in Apex on homogeneous family batches. The held-for-veto clause is superseded; the conjunct is LAW]: a sentiment proposition's stance must be voiced toward us or our solution, never toward the meeting or the occasion, and the blind reader verifies the target as part of the proposition.**

**v3.3 · 5 August 2026 · Two corrections from Matthew, same day, marked in place below. (1) DREAMFORCE IS NEVER A DECISION FACTOR: the clause is struck from v3.2's recorded reasoning; the backburner's reasoning is scale and focus. (2) THE ACCOUNT-GRAIN DELTA IS SMALL, his statement recorded at the contract-set block: one family out and one in, Buyer Role goes away, Decision Orientation comes in, the rest shared; the byte-level wording recovery stands deferred with the grain. The backburner itself is unchanged, and its standing instruction is: accounts are backburnered, never forgotten. Same day, a third note recorded at Ruling 2: call 0's resolution covers the internal side, domain recognition for internal against external, the role map for seller against service, and the resolved record IDs travel in the roster so no downstream stage re-resolves identity.**

**v3.2 · 5 August 2026 · THE ACCOUNT GRAIN IS BACKBURNERED, Matthew's ruling, same day, superseding the write-now half of v3.1 before anything was built. The reasoning, recorded plainly: scale was the stated worry and is not the problem, the read count is bounded; readiness is the problem, zero account-grain gradings, half the AM texts unrecovered, unproven output onto the durable account record, no payoff before Dreamforce. [v3.3: the Dreamforce clause is STRUCK, Matthew's correction, same day. Dreamforce is never a decision factor; the reasoning is scale and focus.] What stands from v3.1: CALL 0, ALWAYS, on every Source, emitting the scope verdict, the opportunity candidate, and the people roster with receipts, resolution consolidated into it; and A PAIR'S SCOPE IS ITS READ'S DECLARED GRAIN as standing law, with exactly one grain dispatching today. What changes: only opportunity-grain family reads dispatch; ACCOUNT and DUAL verdicts route and log, and nothing account-side is extracted, written, or authored; v3.1's Ruling 3 is superseded in place; the AM recovery item and the pair grain-stamp proposal are deferred with the grain; the seller-side flag reverts to a standing non-blocking defect. The five account-grain rows and Matthew's key stay attached to the fixture for the grain's return. The gate is revised in place: scope detection is asserted (Emerson DUAL, B&V 24 June ACCOUNT), the opportunity pipeline's 27-assertion regression set must hold unchanged with call 0 in front, and zero account-grain reads or writes exist anywhere.**

**v3.1 · 5 August 2026 · THE RESOLVER IS RULED: CALL 0 AND THE GRAIN OF A PAIR. Three rulings, Matthew's, argued in session with options and costs before writing, landed in Part II with superseded text marked in place. (1) A PAIR'S SCOPE IS ITS READ'S DECLARED GRAIN: the resolver declares which grains dispatch (opportunity, account, or both); each family read carries exactly one grain's contract set over the whole artifact; a pair inherits scope structurally and no pair ever carries a model-judged scope. The per-claim-versus-per-call open CLOSES; the v0.2 two-sided summary is SUPERSEDED as mechanism (the pairs law requires every quote to byte-locate in the frozen artifact, and a pre-filter that mis-sides a sentence makes the miss unrecoverable); the stretch-stamp law is untouched. (2) CALL 0, ALWAYS: the resolver's model legs execute as a dedicated read on every Source before the sweep, the deterministic ladder feeding it as evidence; it emits the scope verdict, the opportunity candidate or NONE or AMBIGUOUS on the occurred-time window, and the people candidate roster, receipts attached; RESOLUTION CONSOLIDATES INTO CALL 0 and family reads become pure pair emitters; the v2.6 placement bracket is superseded in place; conditionality is recorded as a later measured optimization that preserves the law, never a design fork; call 0's stamp composes charter prose plus the sided ontology text per the two-halves law. (3) ACCOUNT-GRAIN ESTABLISHMENTS WRITE NOW, Matthew's ruling against design's stamp-and-park recommendation, both recorded because the record is the point: the ruled dual-write machinery goes live at the account grain; the account grain's first graded output becomes a gate before any pilot, precision before coverage holding per grain; the seller-side flag is now BLOCKING before the first account-grain write run. The account-grain Political contracts are authored from the recovered AM enumeration, with the three AM-only texts owed from the org's own labels before authoring, never copied from OM bytes. The first fixture is Emerson 17 June with Matthew's key (adjudication sheet §8) plus B&V 24 June as the account-single specimen; the gate asserts five things, and the which-opportunity occurred-time window is NOT ASSERTED on this fixture because the sandbox seed postdates the call.**

**v3.0 · 5 August 2026 · THE FAMILY SWEEP LANDS IN THE CHARTER TEXT, AND THE THIRD GRADING FOLDS. (1) CALL 1 IS A FAMILY SWEEP, ruled by Matthew 5 August: call 1 runs ONCE PER FAMILY, not once per transcript — four bounded reads of the whole artifact, each carrying exactly one family's question set and nothing else. The cost law's exact terms: crossing by PERSON is forbidden because it scales with the roster without bound; crossing by FAMILY is permitted because it is bounded at four or five and scales with nothing — not roster, not length, not density. Asserted in code: the call count must never be a function of participant count. §P8.0's one-sentence job, §P8.0's questions-once letter, the scaling law's "one bounded read," and §P8.1's job sentence are corrected in place below, marked, never deleted. The reasoning, measured and journaled: sentiment holds ONE contract and went 2 → 11 pairs under the sweep, so the scarce resource was ATTENTION, never prompt size; stable establishments tripled (9 → 27 found in both of two runs); and multi-establishment dissolved STRUCTURALLY (7 quotes carrying 16 rows in s1) where an affirmative instruction had produced zero — the failed-twice law applied: change the structure so the wrong answer cannot be expressed. (2) THE TWO-HALVES STAMP COMPOSES PER FAMILY READ: each read's version string covers the charter prose plus that family's declared contract text, per the Board's provenance law; a stamp that does not cover an input that changes the instructions is decoration. (3) THE THIRD GRADING FOLDS at §P8.10, the sweep s1 grading, all 40 establishments, Matthew, 5 August: upheld precision 16 of 17; refusals 21 of 23 correct; zero fabrication; zero UNDERs, ruled no misses found. The one OVER — courtesy upheld as SUPPORTER — is the sixth graded trap: COURTESY IS NOT STANCE; first occurrence post-v2.9, so no wording moves. The two wrong refusals sit on ONE quote and are recorded as standing assertions that fail at verification, examination with CODE. Q10 CLOSES by Matthew's own sweep grades: the annual-total quote carries the criterion "Annual total cost," DC_N and DC_F Formal both True; CODE's don't-build instruction lifts. PARTIAL CRITERIA are ruled under the standing partial-accrual law: UNVERIFIED with receipts, places nobody, projects nothing into the criteria object until elements complete; whether a half-heard criterion surfaces in call-prep guidance is open, later, Matthew's. Record: the adjudication sheet §9; the graded file is preserved as `aao-sweep-s1-graded.csv`.**

**v2.9 · 5 August 2026 · The label-meaning fix, ruled by Matthew from the SECOND JOINT GRADING: the 24 June gate's three MENTOR reads, all graded Neutral — no pair. The defect: the first-draft MENTOR meaning opened with "guides you with insight," true of anyone participating well in a meeting — the ladder's strictest state carried its loosest wording, the same defect class that retired S1, and a reader pushed off under-reading by §P8.9 escaped into it. The fix, §P8.4: THE LABEL MEANINGS ARE NOW PINNED IN THE CHARTER AS LAW, and MENTOR is conjunctive and org-directed — guidance through THEIR ORGANIZATION on our behalf (their politics, their process, their people) or advocacy when we are not in the room; insight about the work itself is participation, not mentoring. New sentence of law: ENGAGEMENT IS NOT STANCE — questions, process suggestions, and "key step" talk voice nothing and emit nothing, and emitting no pair on a warm sentence is correct behaviour, not under-reading. §P8.9's advocacy ruling is unchanged: Jefferson's coaching through his own approval process is mentoring under this line exactly as before. Stakes recorded: mentor-grade evidence crosses to +3, so a cheap MENTOR makes terminals cheap. Nothing else moves.**

**v2.8 · 4 August 2026, evening · The calibration fold, ruled by Matthew from the FIRST JOINT GRADING: the 17 June fixture's review CSV, graded by design under every guard and by Matthew as the standard, before any model ran. The finding: every disagreement was the guarded reader establishing LESS than the grader; zero over-reads; the guards as written produce a reader too conservative to be useful. §P8.9 carries the four rulings: THE CONTENT OF THE CALL, NEVER THE TITLE, IN BOTH DIRECTIONS (behaviour on the call establishes: a person doing evaluation is an Evaluator whatever their title or level); ONE QUOTE, SEVERAL ESTABLISHMENTS (call 1 emits one pair per establishment a sentence voices, instructed affirmatively); ADVOCACY IS NEVER DISCOUNTED FOR COMING FROM A PROFESSIONAL (the neutral disposition guards against pleasantries, never against a buyer coaching the seller through her own organization's process — the guard as written was eating the most load-bearing sentiment evidence a call carries); and THE FAMILIES ARE NOT EQUALLY HEAVY (Support, Political Status, Buyer Role lead; criteria are sparse by nature, §P8.5 tightened: buying-process mechanics and approval coaching are never criteria, a call producing many criteria is a calibration alarm; Decision Orientation waits for the account-level charter). No shape changes: three calls, pairs, no abstention rows, the counter, all as v2.7 ruled. The first answer key: Matthew's grades on the fixture stand as the standard the run is measured against; agreement specimens recorded (Jefferson Outside Political Structure, Neeraja Political Structure and Supporter). THE COUNTER ARITHMETIC IS REFINED IN THE SAME FOLD, Matthew's ruling: each distinct verified establishment moves the counter one in its direction, a call's movement is the net sum, and the only caps are the clamp at ±3 and the ceiling of the strongest state the evidence voices — supporter-grade evidence tops at +2, mentor-grade reaches +3, symmetric on the negative side — so TERMINALS ARE PROOF-GATED, NEVER TIME-GATED: Mentor or Enemy is reachable in one call over multiple extreme-grade establishments or an explicit declaration, mixed evidence nets with every quote journalled on its own side, movement applies to the standing counter read at reconciliation, and the grading prices the quote while the arithmetic only adds (§P8.4). No model has run yet: the latency and rumination questions stay open until the 24 June gate run, and no number enters any document before it is journalled.**

**v2.7 · 4 August 2026, afternoon · The simplification fold, all Matthew's rulings, argued in session and settled before writing. §P8 IS REWRITTEN WHOLE. (1) NO ABSTENTION ROWS EXIST, at any grain, ever: the system journals what happened, never what did not; the v2.6 abstention vocabulary (`not_surfaced`, `not_in_excerpts`) is retired unbuilt; auditability is the run receipt, one row per Source per pass, plus one-for-one-for-one arithmetic enforced in code between the stage row sets. The silence hazard dissolves: the system no longer asserts silence anywhere. (2) THE PAIR SHAPE: call 1 reads the whole artifact once, resolves deal, account, and candidate people (candidate opportunities windowed on the EVIDENCE-OCCURRED clock, open plus recently closed as of the call, never the run date), and emits question-answer pairs, verbatim words plus meaning, NO PERSON ATTACHED; call 2 identifies only, attaching each pair's person from the closed candidate list or saying NONE or AMBIGUOUS, never re-judging, never reading the whole transcript; call 3 verifies blind, unchanged. Judgment of what was said is call 1's; who said it is call 2's; whether the words carry it is call 3's; the speaker-rank laws stay in the gates after identification, which is the existing judge-words-first law made physical. (3) THE PAIR LEDGER: every pair persists twice, the initial unowned row from call 1 and the linked identified row from call 2, so every stage is measurable in isolation; the initial row's verbatim string, byte-located in the frozen artifact, IS the anchor, and the v2.6 anchor ledger is retired as a separate thing. (4) TRUE-OR-NOTHING: every question points one way; no FALSE, no inversion, no two-sided verdict anywhere in the pass; negative states are established by their own affirmative words (the vendor authored them); the two-sided Political questions split into one-way halves; the TRUE/FALSE verdict vocabulary leaves the pass. (5) THE SUPPORT ONTOLOGY IS RETIRED FROM EXTRACTION and the sentiment counter is RE-PROMOTED from fallback to mechanism (§P8.4): the model reads sentiment directly into five states with one-line label meanings; per call per person the counter moves one step toward the voiced state and never past it; explicit declarations jump with the sequence flagged; mixed evidence nets to no move with every quote journalled; the nine questions stay recorded byte-exact as the vendor's asset. Reasoning recorded in §P8.4: the quiz missed the obvious (the Adam UNDER), over-read the vague (both S1 OVERs), and made the model ruminate; ontology-where-feasible survives with its application corrected. Political, Buyer Role, criteria, insights, Process, and assessment keep their ontologies; Coverage stays computed. (6) Partial-evidence accrual survives for fact families; sentiment reads are whole, and accumulation is the counter's job: two accrual mechanisms, both receipts-backed. (7) The test fixture: the 17 June Emerson/AspenTech call (VideoCall 6qrV4000000J5K9IAK, 41 minutes, five participants, criterion-dense, two participants with no Contact link including Jefferson Vargas, ECI-stamped to a deal that was the account's only open opportunity at call time and has since closed won while a renewal opened, making it the standing occurred-time-resolution specimen); the harness satellite `aao-pass-test-harness-v1_0.md` carries the test suite. Glossary still deliberately held; owed terms listed at §P8.8.**

**v2.6 · 4 August 2026 · The charter-writing fold. THE OWED TEXT LANDS: §P8 carries the three call charters (locate, attribute and claim, verify), the decision-criteria contracts joining People, and every settlement the v2.5 head assigned to this writing, each with its reasoning: S1 RETIRED from the claim path (§P8.5); the ABSTENTION VOCABULARY settled (§P8.0: `nobody_said` retired as writable, `not_surfaced` and `not_in_excerpts` enter, each naming its authority, transcript-wide silence derived and never stored); the VERDICT SET settled including FALSE's reachability and the two-sided verdict set landing in a body at last; ANCHOR MECHANICS settled (verbatim strings by exact byte match, never line numbers); call 2's CONTEXT WINDOW settled as mechanism with W measured, never carried in a document; the RECALL GATE settled (strip-and-restore on the 24 June fixture against Matthew's grades plus the canonical criterion). DC-F's TYPING AXIS RULED by Matthew this day: the research defines formal and informal, and the research's axis is provenance (organizational standing spoken, else Informal, upgradeable). The research grounding is the satellite `aao-decision-criteria-research-v0_1.md`, written this session first, as the seed required. Architecture v2.1's criteria-emitted-by-the-insight-charter placement is superseded (§P8.4). §P7.3 is superseded as the pass shape and marked in place; its surviving machinery is named in §P8. Owed onward: Glossary terms, Model & Flow's abstention enum and anchor-ledger entity, the Board's fold at session end (§P8.7).**

**v2.5 · 4 August 2026 · The three-call fold, at the head, owed to the body at its next bump. THE PASS IS REDESIGNED BY MATTHEW'S RULING: the three-call shape supersedes §P7.3's one-read extract as the pass shape. Call 1, LOCATE: one read of the whole artifact, extremely wide, resolving which deal, which account, which people and which target records the call concerns, and emitting anchors for every place a chartered consequence was spoken (support movement, political movement, decision criteria, every declared family). It makes no claims, extracts no evidence spans, renders no verdicts. Call 2, ATTRIBUTE AND CLAIM: reads only the excerpts call 1 anchored, with a measured amount of surrounding context, and makes claims against the IDs call 1 resolved, using the declared charters' contracts. It never reads the whole transcript. Call 3, VERIFY: the blind reader, unchanged: separate smaller model, claim and evidence only. EACH CALL'S CHARTER FORBIDS THE OTHER CALLS' JOBS, because the failure mode is a call quietly taking back a job that was taken from it. QUESTIONS ARE ASKED ONCE PER TRANSCRIPT, NEVER PER PERSON: the per-person fan-out (16 questions × 3 people as 48 contract-questions in one prompt) is retired as the read's shape; person resolution is call 1's job, person binding call 2's. THE DECLARED-SET LAW RE-SCOPES, amending §P7.3's first-binding sentence in place: it binds the claim path, so call 2's contracts are exactly the declared charters' and never the whole cabinet, while call 1 reads wide by this ruling; the violation remains handing the model an undeclared CLAIM question, and a wide locate is not one. DYNAMIC SCALING IS MEASURED, NEVER ASSUMED: call 2 splits into further calls when a measured group approaches budget, the blind reader may split the same way, so a dense transcript costs six calls where a light one costs three and no call approaches the 120-second ceiling. THE BUDGET LAW, Matthew's domain ruling, reasoning recorded: cost scales with transcript length and never with roster size, because consequence density is bounded by duration and generally anti-correlates with headcount (crosstalk, drift and demonstrations thin it; the deepest conversations are the smallest), so 45 minutes with seventeen people carries approximately the consequence of 45 minutes with three; attribution volume may grow with people, consequence does not. DECISION CRITERIA JOIN PEOPLE: relationship-map territory only, formal and informal per Altify's own object (ALTF__Decision_Criteria__c: Type Formal/Informal, Subject, Milestone, Required, criterion-contacts child), a criterion is owned by the person who voiced it, and "what I'm trying to do is just eliminate clicks" is the canonical informal case this build produced and missed by declaration. THE CHARTER TEXT IS OWED, NOT WRITTEN: decision-criteria recognition rules are researched first (next session's first act) and the three call charters are written from that research in one session, so the text and its grounding share a context. POST-RUN FINDINGS, first real adjudication (B&V 24 June, six answers, Matthew, sheet v0.3): four PASS, two OVER, both wrong-scope, both S1, over-reads perfectly concentrated in one proposition, so the finding is S1's text, not the model and not the reader's size; S1 is three ways bad (places nobody, TRUE for everyone, majority wrong) and is first candidate for rewrite or retirement at the charter writing; the FIRST RECORDED UNDER is S2 for Adam Meloan, the only Support question with placement power, never surfaced by the read, written as nobody_said, which is why the person Matthew grades an obvious Supporter carries no Status; findings=1 gains its first structural explanation with direct evidence: the read establishes gate questions that place nobody and misses placement questions. An ontology finding, not a model finding. THE ABSTENTION VOCABULARY IS OPEN, sharpened by the redesign: nobody_said claims the transcript is silent when what is established is that this read did not surface it, and under the three-call shape call 1's misses become everyone's silence; two reasons where there is one, settled at the charter writing. Charter 1.1.0's coverage rule stands as history: it did not repair the one-call read's instability, and the redesign is the response, not a seventh dial. The two-sided verdict set (establishes-false on two-sided propositions, recorded in the adjudication sheet 3 August) never landed in §P7.3's body: the decision-slot hazard by name; the verdict set, including FALSE's reachability, is settled at the new charters' writing rather than patched.**
...
**Reading order for a single-purpose session:** the Board first, then this document's section for your focus: **§P8 the pass charters (the authoritative pass shape)**, §P1 People, §P2 Problems, §P3 Politics (open), §P4 Process, §P5 Scope Resolver, §P6 Account-level ontology, §P7 Harness briefs (§P7.3 superseded as shape by §P8).
...
---
## §P8 · The pass charters · charters v3.0, amended in place through v3.5; the brackets carry each amendment's version · the three calls, swept and calibrated

**Status: the authoritative pass shape and the charter text. This section was first written at v2.6, REWRITTEN WHOLE at v2.7 by Matthew's rulings of 4 August, and amended at v3.0 by the family-sweep ruling of 5 August, corrected in place below. §P7.3 remains superseded. Everything marked SETTLED carries its reasoning; Matthew corrects in place. Everything marked MEASURED is a number no document carries: it lives in BUILD_JOURNAL, set by measurement.**

### §P8.0 · Law common to the pass

**The three calls, one sentence each.** Call 1, LOCATE AND PAIR: one read of the whole artifact; resolves which deal, which account, and the candidate people; emits question-answer pairs, each pair a question from the declared set plus the verbatim words that answer it plus the meaning those words voice, with no person attached. **[v3.0: the first clause is superseded — CALL 1 IS A FAMILY SWEEP, ruled by Matthew 5 August: call 1 executes once per declared family, four bounded reads of the whole artifact, each carrying exactly one family's question set and nothing else. Everything else in the sentence stands: pairs, verbatim words, voiced meaning, no person attached. The reasoning and the measurements are the v3.0 head's; §P8.1 carries the corrected job.]** Call 2, IDENTIFY: for each pair, and only for pairs that exist, attaches the person it belongs to from call 1's closed candidate list, or says it cannot. Call 3, VERIFY: the blind reader, unchanged in kind: sees one identified claim and its quote and says whether the words carry it. Everything downstream (gates, speaker requirements, arithmetic, Answers, projection, watermarks, citation law) is code, not model. **[v3.1: A CALL 0 PRECEDES THE SWEEP, the resolver's read, ruled by Matthew 5 August. It routes and resolves and never establishes: the scope verdict (opportunity, account, or dual), the opportunity candidate or NONE or AMBIGUOUS, and the people candidate roster, receipts attached. Code dispatches each declared grain's family reads from its verdict. Part II carries the rulings and the charter.]**

**The prohibition law.** Each call's charter forbids the other calls' jobs. Call 1 never names a person. Call 2 never finds new evidence, never re-judges the pair's meaning, never reads the whole transcript. Call 3 sees nothing but the claim in front of it. A charter change that softens a prohibition is a shape change and re-runs the recall gate.

**Questions once per transcript, never per person.** The declared question set appears exactly once, in call 1's prompt. No prompt anywhere enumerates a person-crossed-with-question product. A question nobody answered produces nothing: no pair, no row, no model thought, nowhere, ever. **[v3.0: the letter corrects with the sweep — the declared set appears exactly once PER PASS, distributed one family per read; "call 1's prompt" is now four prompts, each carrying one family's set and never another's. The substance is unchanged: no per-person product anywhere, and a question nobody answered still produces nothing.]**

**No abstention rows exist. RULED by Matthew, 4 August.** The system writes what happened, never what did not, the same way a good human keeps a journal. The v2.6 abstention vocabulary (`not_surfaced`, `not_in_excerpts`) is retired unbuilt; `nobody_said` stays retired; no per-unit silence row is ever written at any grain. What replaces it, and it already exists: **the run receipt**, one row per Source per pass: read on this date, by this charter version, these calls made, these pairs found, these claims written. That receipt is the answer to "did you even look," it feeds the flag's last-evidence-considered timestamp, and it is one row instead of a hundred. Transcript-wide silence about any question is derivable on demand from the pair ledger and is never stored as an assertion. The silence hazard dissolves with this: the system no longer asserts silence anywhere, so no read's reliability is ever borrowed to claim it. **[v3.0: the receipt's load rises under the sweep — four reads per pass make a partially completed pass expressible, and the receipt is what records which family reads ran. The object remains owed by CODE; `assertOneForOne`'s HELD / INCOMPLETE / BROKEN return and the per-family `UNREACHABLE` line carry the gap until it lands.]**

**One for one, for one. SETTLED.** Every pair call 1 emits receives exactly one disposition from call 2 and, if identified, exactly one verdict from call 3. This is arithmetic enforced in code between the row sets, and a mismatch throws loudly: it is always our bug, never a model behaviour, per the parse law. No rows are written to prove completeness; the counts are the proof.

**The pair ledger: two rows per pair, initial and identified. RULED by Matthew, 4 August.** Call 1's output persists as it was found: question, verbatim answer words, meaning, full or partial (fact families only, per below), scope. Unowned. Call 2's identification persists as a separate row linked to the first. Two rows because every stage must be measurable on its own: pairs found, pairs identified, identifications refused, verifications refused, each a count between two row sets, and any stage's defect is visible in isolation. The identified row is what call 3 verifies and what the gates promote toward Claim. **This replaces the v2.6 anchor ledger entirely**: the verbatim answer string on the initial row, located in the frozen NF1 artifact by exact byte match, is the anchor; there is no separate anchor entity. Collapsing the two rows into one is a decision for after the system is proven, not now. Entity ruling owed to Model & Flow; CODE proposes the mechanism first.

**Verbatim strings by exact match, never line numbers. SETTLED, carried from v2.6.** The answer words on a pair are verbatim, character for character, byte-checked against the frozen artifact. Zero matches discards the pair and increments a counter (unit: pairs). Multiple matches: all occurrences stand; the words were said more than once.

**The scaling law, unchanged in substance.** Cost scales with transcript length, never roster size. Call 1 is one bounded read. **[v3.0: corrected — call 1 is FOUR bounded reads, one per declared family; the read count follows the family count, bounded at four or five, and is never a function of roster or length. The cost law's exact terms are the v3.0 head's; the code asserts the call count is never a function of participant count.]** Call 2's input is pairs plus the candidate list plus a measured window of surrounding turns per pair for context; it is small by construction and splits by pair batch when measured input approaches the MEASURED threshold; call 3 splits per claim batch the same way. No call retries by growing. A quiet call costs three calls; a dense one costs a few more; the count follows how much of consequence was said. **[v3.0: call 3 is the only stage whose cost is linear in harvest size against a hard platform ceiling — forty claims at one per callout exceeds the 120-second transaction budget and batches caller-side; placement and the ceiling arithmetic are Architecture's.]**

**Telemetry.** Every counter names its unit at the point of print. Minimum set: pairs emitted, located, discarded (pairs); identifications made, refused (pairs); claims upheld, refused (claims); per-call latency, input, output, thinking share, cacheRead. Thinking share is watched deliberately: the rumination this shape exists to kill is measurable, and the June fixtures show whether it is gone.

**The recall gate. SETTLED, re-aimed.** Before anything downstream is trusted, call 1 runs against the 24 June B&V fixture and the target set from Matthew's grades: a pair must exist for the words behind each graded PASS, for the S2/Adam placement words, and for the canonical criterion. Coverage printed as targets covered over targets (unit: targets). The same run asserts no pair reproduces the two graded over-reads. The gate re-runs on any call 1 charter change and any window change. **[v3.0: the gate's standing successor is the REGRESSION SET, ruled 5 August, the Board's law: every establishment Matthew has graded PASS is a standing assertion on every run; every graded trap must not appear; correct refusals are neither. Keyed on contract plus byte-range intersection, never the stored string. The sweep grading seeds it with 18 assertions and one trap; q9's flickering PASS is its first expected report.]**

**Models. MEASURED.** Calls 1 and 2 on the strong model (call 2 may prove cheap enough to downsize; measured, not assumed); call 3 on the smallest model that passes its re-comparison gate on the new claim shape. Tiers are config; no model name is law. **[v3.0: the two-halves stamp law binds every read — a version string covers the charter prose plus the declared contract text that composes the prompt, and under the sweep it composes PER FAMILY READ. A charter whose text changes while its stamp does not is a provenance break; the law is the Board's, carried to Architecture at its bump.]**

### §P8.1 · Call 1 · LOCATE AND PAIR

**The job.** One read of the whole normalized artifact. Three outputs, all proposals, all consumed by code. **[v3.0: THE FAMILY SWEEP, ruled by Matthew 5 August — call 1 executes once per declared family: each execution is one read of the whole normalized artifact carrying exactly that family's question set and nothing else. The pass's call 1 is the set of family reads. What this bought, measured: attention. Sentiment holds one contract and went 2 → 11 pairs when the read stopped being asked everything at once; stable output tripled; and one utterance now pairs independently in several families' reads, which is where multi-establishment lives structurally — the affirmative instruction it replaced had produced zero doubles twice, and the failed-twice law retired the instruction in favour of the structure. Resolution's placement among the reads is CODE's mechanism, journaled, not charter text.]** **[v3.1: that last sentence is superseded. Resolution consolidates into call 0, the resolver's read; family reads are pure pair emitters; placement is charter law now, Part II.]**

**Resolution.** Which account, which opportunity, what scope, per the resolver's shapes: each answered candidate, NONE, or AMBIGUOUS, with the deterministic ladder and the two-key lock unchanged in Apex around it. **The candidate set is built as of the evidence-occurred clock, not the run clock: open opportunities plus recently closed, at the time the call happened.** The 17 June Emerson fixture is the standing specimen: at call time the account held exactly one open deal; by the first test run that deal had closed won and a renewal had opened, so a candidate set built naively today resolves the call to a deal that did not exist when the words were spoken. And the people: every participant and every load-bearing mentioned person, matched against the closed candidate set (map rows, Contacts, shadows, roster), NONE or AMBIGUOUS where honest, never invented; linking and shadow creation stay the gates' work.

**Pairs.** For every place the words answer a declared question: the question, the verbatim words, and the meaning the words voice. **One utterance usually establishes more than one thing, and call 1 emits one pair per establishment** (ruled at §P8.9): a buyer coaching the seller through her own approval process is, in one breath, voicing sentiment, placing herself politically, and exercising a buying role, and the charter instructs the model to emit each rather than pick the best one. **[v3.0: under the sweep this is structural — each family's read pairs the same bytes independently, and the instruction survives only within a family.]** **True-or-nothing, in the question's own direction. No pair ever carries a false, an inversion, or a two-sided verdict; every question points one way, and words either voice it or produce nothing.** For the fact families (Political, Buyer Role, criteria, Process, assessment), a pair also says full or partial: full when the words carry every element, partial when they carry some, and partial is the instructed default under doubt, because a partial accrues safely and places nobody. For sentiment there is no partial; see §P8.4.

**Forbidden to call 1.** No person is ever named on a pair. No placement, no rung, no aggregate judgment about anyone. Nothing about what was not said. **[v3.0: and no read ever carries another family's questions — cross-family judgment in one read is structurally unexpressible, the fourth application of the failed-twice law.]**

**Schema, as designed.** `AAO_Locate` 2.0.0: a `resolution` object, a `people` array (the discovery-body shape), a `pairs` array (`question_code` runtime-closed, `answer_text` verbatim, `meaning` runtime-closed per family, `coverage` full or partial on fact families). Dumped from the org after build, per the envelope law.

### §P8.2 · Call 2 · IDENTIFY

**The job.** For each pair, whose words are these and, where the question asks about a person other than the speaker, who is it about. Input: the pairs, the closed candidate list from call 1's resolution, and a measured window of turns around each pair's location for context. Output: per pair, exactly one of: a person from the closed list, NONE (belongs to nobody resolvable), or AMBIGUOUS (two candidates and the context cannot separate them). AMBIGUOUS routes to the Identification flag through the existing gates.

**Forbidden to call 2.** It never reads the whole artifact. It never re-judges whether the words answer the question or what they mean; a pair it believes is wrong is still identified, and call 3 exists to kill it. It never emits a pair. It never invents an identity outside the closed list.

**Attribution rules, carried forward.** Ownership of spoken words is the speaker; a criterion belongs to its voicer; a reported holder earns a person row through the person-row boundary but no ownership on another's report. Speaker-rank and subject-person requirements are enforced by the gates after identification, never by call 2, which is the standing law (judge the words first, adjudicate the speaker after) made physical.

**Schema, as designed.** `AAO_Identify` 1.0.0: `dispositions` array, exactly one entry per handed pair, in order: `pair_ref`, `person` (runtime-closed or NONE or AMBIGUOUS), `basis` (one sentence: spoken by them, said about them by name, answered when addressed). Dumped after build.

### §P8.3 · Call 3 · VERIFY

Unchanged in kind: separate smaller model, sees one identified claim (question, meaning, person-agnostic content) and its verbatim quote, nothing else, and says whether the words carry the claim. For sentiment claims the check includes scope: the quote must carry that feeling toward us and our solution, because both graded over-reads were right-sounding words about the wrong thing. Its adjudicated comparison gate re-runs on the new claim shape before any size is trusted. Splits per claim batch. Forbidden: everything except the claim in front of it. **[v3.0: CALL 3 VERIFIES THE PROPOSITION AND NOTHING ELSE, the first Emerson grading's repair made law — a placement is what a true proposition points at, never something the words must also voice; the placement string never reaches the blind reader, gated by `carriesChoice()`, true only where a family has more than one legal meaning.]**

### §P8.4 · Sentiment · the ontology retired, the counter re-promoted · RULED by Matthew, 4 August

**The ruling.** For the Support dimension, the nine-question extraction quiz is retired. The model reads sentiment directly, which is its native strength: the words voice one of five states, mentor, supporter, neutral, non-supporter, enemy, each carrying a one-line meaning drawn from Altify's own value descriptions, as label definitions and never as interrogation checklists. "Honestly, we love what you've built" is a supporter reading, done. The reasoning, recorded: the quiz made the model cross-examine every warm sentence against nine propositions and second-guess each one, which is the measured rumination; its strictest question set a bar no real buyer meets, because procurement never voices exclusive preference out loud, so it generated the fixture's one UNDER (Adam, the obvious supporter, missed on wording); and its vaguest question generated both OVERs and was already retired. The quiz's record is: misses the obvious, over-reads the vague. Matthew's grades are the standard, and his grades are sentiment reads.

**The label meanings are law, pinned here at v2.9 from the second joint grading (Matthew's grades: three MENTOR reads on the 24 June gate, all graded Neutral — no pair).** The one-line meanings the model receives are these, verbatim, and nowhere else: **MENTOR:** their words guide you through *their own organization* on your behalf — their politics, their process, their people — or advocate for you when you are not in the room; insight about the work, the product, or the project is participation, not mentoring. **SUPPORTER:** prefers you and says so. **NON-SUPPORTER:** prefers an alternative, including doing nothing. **ENEMY:** works against you, or believes your success harms them. **No neutral label exists: words that voice no stance produce no pair, and ENGAGEMENT IS NOT STANCE** — questions, process suggestions, "I think this is a key step," a person doing their job well and warmly, all voice nothing and emit nothing; emitting no pair on a warm sentence is correct behaviour, not under-reading. The defect the second grading caught, recorded: the first-draft MENTOR line opened with "guides you with insight," which is true of anyone participating well in a meeting — the ladder's strictest state carried its loosest wording, the same defect class that retired S1 (true for everyone), and a reader pushed off under-reading by §P8.9 escaped into it. The stakes that made it urgent: mentor-grade evidence is what crosses to +3, so a cheap MENTOR makes terminals cheap. §P8.9's advocacy ruling is unchanged — Jefferson's coaching through his own approval process is mentoring under this line exactly as before; "this is a key step" never was. **[v3.0: the third grading adds the courtesy sibling — COURTESY IS NOT STANCE. "Looking forward to it" on a working session was upheld as SUPPORTER and graded False ("this is niceties"); it is the sixth graded trap. The neutral disposition's scope (§P8.9) is unchanged: it guards against pleasantries and courtesy exactly as written; the trap is its enforcement specimen. First occurrence post-v2.9; per the failed-twice law no wording moves until a second.]**

**The scope of the retirement, precise.** Sentiment only. Political Status keeps its ontology because it is facts, not feeling (approves budgets, defines goals, brokers information), with its two-sided questions split into one-way halves, each pointing at its own placement, per the ceiling derivation. Buyer Role keeps its authored definitions. Decision criteria keep the recognition rules from the research satellite. The insight families keep theirs. Coverage stays computed. The nine Support questions stay recorded byte-exact as the vendor's asset and the wizard's content; they are no longer what any model is quizzed on. **The governing principle survives with its application corrected: ontology where ontology is feasible; sentiment is where it is not, and the fallback deliberately preserved for exactly this case now runs it.**

**The counter, re-promoted from fallback to Support's mechanism, arithmetic refined at this fold from the first joint grading (Matthew's ruling: terminals are proof-gated, never time-gated).** One integer per person, minus three to plus three, clamped, with the rungs at their ruled positions: +3 Mentor, +1 and +2 Supporter, 0 Neutral, −1 and −2 Non-supporter, −3 Enemy. **Each distinct verified establishment moves the counter one, in its direction; a call's movement is the net sum of its establishments.** Two caps do all the governing: the clamp at ±3, and **evidence never carries anyone past the strongest state it actually voices** — supporter-grade words top out at +2 however many there are, mentor-grade words can reach +3, and symmetrically −2 and −3 on the negative side. So one warm quote is +1; a call stacked with supporter-grade evidence is +2 and solidly placed; and Mentor or Enemy is reachable in a single call over multiple extreme-grade establishments, because time was never the gate, proof is. At +2, further supporter-grade evidence does nothing, since supporter's ceiling is +2 and only mentor-grade words cross it; one −1 pulls to +1, still Supporter. **Mixed evidence nets** (+2 positive and −1 negative is +1), every quote journals on its own side, nothing averages, and neutral stays derived, never written. **Movement always applies to the standing counter, read at reconciliation before anything writes** — the current-state check that has been in the pipeline since the beginning. **The jump rule stands:** words that explicitly declare a state outright set the counter to the declared position regardless of count, and a contention flag carries the whole sequence with quotes whenever movement reverses standing polarity. Replay over claims in evidence-occurred order reconstructs the counter exactly; determinism holds because the counter is a clamped sum over receipts. **The grading is what prices a quote; the arithmetic only adds** — the first fixture is the specimen: quotes design read as mentor-grade, Matthew graded inside supporter, and his grades through this arithmetic produce exactly his answer, +2, top of Supporter.

**Two accrual mechanisms, both receipts-backed, and the line between them.** Fact families accrue through partial evidence: a half-answered question writes its quote as an open finding (UNVERIFIED with receipts, standing law) and later evidence completes it. Sentiment accrues through the counter: each read is whole, never graded into partial supportiveness, because partial sentiment is the second-guessing this ruling kills; accumulation across calls is the counter's job. Neutral is never written; it is the derived reading when nothing stands, which is the no-abstention ruling applied to the map.

**What stands guard, since sentiment reading is looser than a quiz.** The blind reader checks every quote carries the claimed feeling toward us (the wrong-scope guard). The neutral disposition survives as charter text with its scope CORRECTED at §P8.9: it guards against pleasantries and courtesy, and it never discounts advocacy or coaching behaviour, whatever the speaker's function. A procurement lead telling the seller what his approvers will smile upon is not being polite; he is helping us win, and the read says so. No rate target ever enters the instructions. Human override, watermarks, no-decay, and the contention machinery are untouched. And the thinking-share telemetry plus the June fixtures measure whether the rumination actually died; if it persists, the fix is instruction wording, not another shape change. **[v3.0: measured and closed — the Board's stamp carries it: the speed and rumination questions are closed on the June fixtures.]**

### §P8.5 · The decision-criteria contracts · TIGHTENED at v2.8

DC-N (naming: identifiable condition, stance, decision linkage), DC-F (typing by provenance, Matthew's ruling: organizational standing spoken makes Formal, else Informal, upgradeable), DC-R (gate language only, default unchecked).

**The decision-linkage element is sharpened, from the first joint grading (§P8.9): the condition must bear on choosing or judging the solution or vendor.** Buying-process mechanics are never criteria: questionnaires to complete, documents to package, review sequences, who sends what to whom — that is Process-family material and pairs with the Process questions, not with DC-N. Coaching the seller on how to win internal approval is never a criterion either; it is sentiment and political evidence about the coach. **Criteria are expected to be few and far between, often zero on a call, and important precisely because they are rare: a call producing many criteria is a calibration alarm, not a rich harvest.** The first graded fixture is the specimen: a wide reading produced thirteen criteria candidates and Matthew's grading dissolved nearly all of them into people establishments and process facts. Grounded in the research satellite; the marker families are call 1's listening guide for the criteria family; the negative rules bind call 1's pairing (seller words never name a buyer criterion, a buyer's question is not a naming, a stated goal does not create a criterion, current-state description carries no stance). Ownership derived from the identified speaker, junction rows voicer-only. Projection per the satellite's field map and the org's own conventions. Criteria writes bypass ratification and never flag, standing law. Architecture v2.1's insight-charter emission placement stays superseded; call 1 is now the one reader holding both taxonomies, so the arbitration argument is satisfied structurally. **[v3.0: two amendments from the third grading. PARTIAL CRITERIA run under the standing partial-accrual law — a partial DC pair accrues as an open finding, UNVERIFIED with receipts, places nobody, and PROJECTS NOTHING into `ALTF__Decision_Criteria__c` until later evidence completes the elements; a graded True on a partial row asserts location and meaning, never fullness; whether a half-heard criterion surfaces in call-prep guidance is open, later, Matthew's. And the calibration alarm has its first calibrated reading: Emerson s1 produced six criteria, all six graded True — the fixture is criterion-dense by selection, so six on THIS call is harvest, not alarm; the alarm remains the rule for ordinary calls.]**

### §P8.6 · Superseded by this section's own history, named

From v2.6, reversed at v2.7 by Matthew's rulings: the abstention vocabulary (retired unbuilt; no silence rows at any grain); the separate anchor ledger (the initial pair row is the anchor); call 2 as judge (call 2 identifies only; judgment of meaning is call 1's, verification call 3's); the TRUE/FALSE verdict vocabulary in the pass (a pair's meaning is the state or answer its words voice, one direction, true-or-nothing); the two-sided guard machinery in the pass (two-sided questions split into one-way halves); and the Support quiz as extraction contracts (§P8.4). From v2.6, surviving: S1's retirement (subsumed: the whole quiz retired for extraction), the recall gate, byte-exact verbatim matching, the scaling law, questions-once-per-transcript, the prohibition law, partial-evidence accrual for fact families, and the criteria contracts. **[v3.0 adds to the superseded list: call 1 as ONE read of the whole artifact (v2.7's shape, superseded by the family sweep, 5 August) and the affirmative multi-establishment instruction as the mechanism (superseded by the sweep's structure; the instruction survives within a family's read).]**

### §P8.7 · What is unverified, named

Call 1's reliability at whole-transcript pair extraction: the shape that failed before was a whole-transcript read, so the recall gate is load-bearing, not a formality. **[v3.0: precision is now measured twice — the first grading (14 rows) and the sweep grading (40 rows, 16 of 17 upheld True). RECALL remains the open half: q9's establishment appears in 2 of 4 runs and is absent from graded s1; the regression set is the instrument; partitioning the artifact is the named, unproposed lever.]** Call 2's identification accuracy against a real roster. The sentiment reads against Matthew's grades: over-reading rate under the looser standard, measured on the June fixtures before trust. **[v3.0: measured on s1 — one courtesy OVER of seventeen upheld; the trap stands.]** Whether the rumination died: thinking-share telemetry. **[v3.0: closed; the Board's stamp carries it.]** The window, the split threshold, the small model's fitness. The Apex-side transcript retrieval action (capability law: unverified until called from Apex). Every one measured before trusted, journalled when it is.

### §P8.9 · The first calibration · RULED by Matthew from the first joint grading, 4 August

The review CSV on the 17 June fixture was graded twice before any model ran: design's read under every guard this document carries, and Matthew's read as the standard. **Every disagreement ran the same direction: the guarded reader established less than the grader.** Not one over-read. A reader holding all the guards lands too conservative to be useful, and a model holding them will too. Four rulings follow, and they bind call 1's instructions directly.

**The content of the call, never the title, in both directions.** Half of this was always law: a role is never inferred from a title. The other half is now law too: **behaviour on the call establishes.** A person doing evaluation is an Evaluator whatever their title; a person routing the case to the deciding body is placed by that act; an entry-level person put on the project who evaluates, evaluates. The guard against titles was never a guard against evidence.

**One quote, several establishments.** The consequential sentences in a call rarely establish exactly one thing. "I'm going to support whatever it's going to take to make sure that it gets through" is sentiment and political placement at once, and both pairs are emitted. Picking the single best reading of a rich sentence is under-reading wearing discipline's clothes.

**Advocacy is never discounted for coming from a professional.** The neutral disposition guards against courtesy: greetings, thanks, pleasantries, the warmth any buyer shows any vendor. It does not discount a buyer who coaches the seller through her own organization's process, tells us what the approvers need to see, or moves our case internally. That is the most load-bearing sentiment evidence a call can carry, and the first graded fixture showed the guard, as written, eating it.

**The families are not equally heavy.** Support, Political Status, and Buyer Role are the product on the relationship map, and the pass's attention belongs there. Criteria are sparse by nature (§P8.5) and expected to come back empty on most calls. Decision Orientation joins at the account-level charter, not here. A read that returns many criteria and few placements has it backwards.

### §P8.10 · The third calibration · the sweep grading · graded by Matthew, 5 August · charters v3.0

The family-sweep shape's first grading and the gate on the opportunity grain, passed on precision. Run s1 on the 17 June Emerson fixture: 40 establishments, all graded, inverted method. The record is the adjudication sheet §9; the graded file is `aao-sweep-s1-graded.csv`; the rulings travelled as Addendum 15. What binds this document:

**The gate result.** Upheld precision 16 of 17; refusals 21 of 23 correct; zero fabrication; zero UNDERs, ruled no misses found. One wrong establishment would have landed on the map, and it is a courtesy read, not a fabrication and not a misattribution. Against the first grading's 2 clean passes of 14, this is the four repairs plus the sweep, visible in one table.

**Courtesy is not stance — the sixth graded trap.** Ryan Couture's "…Looking forward to it" upheld as SUPPORTER, graded False: "This is niceties." Bytes at offset 35552. The neutral disposition's text is unchanged; the trap enforces it. First occurrence post-v2.9; the failed-twice law holds any wording change until a second.

**The wrong-refusal residue.** poq7 and seq2, one quote (Neeraja: "I just want to make sure that if anything, I help support… Jeff, I work with on this one, Ryan, this is his first"), graded True as POL_PS3 and MENTOR; call 3 refused both on direction grounds. Recorded as standing assertions that currently fail at verification — the defect they instrument. Examination with CODE; no wording change until the shape fails twice.

**q10 closed.** Jefferson's annual-total quote carries the criterion "Annual total cost," DC_N and DC_F Formal, by Matthew's own sweep grades on the same bytes (deq1/deq2, True). The don't-build instruction lifts. The same bytes carry a refused BR_EVAL — one quote, several establishments, landed structurally, with the families disagreeing correctly.

**Partial criteria, ruled.** The standing partial-accrual law governs; §P8.5 carries the amendment. Nothing new is built.

**The seller-side specimens.** Both Altify-side rows (Renee, Wendy) graded INTERNAL by Matthew and already refused by call 3 on other grounds. The sandbox flag stands as recorded, CODE's, deliberately untouched.

**What the grading does not say.** Zero UNDERs is not zero misses: q9's establishment (Flexibility of Services Hours) is absent from s1, per its 2-of-4 record. Recall stays the open problem; the regression set is its instrument.

### §P8.8 · Owed onward

To the Glossary at its next bump: pair, initial and identified rows, run receipt, sentiment read, the counter's return, the retired terms marked retired. **[v3.0: paid at Glossary v2.5, with family sweep, regression set, and the third state added.]** To Model & Flow: the pair-ledger entities (CODE proposes the mechanism first), the removal of the abstention-row machinery from the field tables, the counter's field home. **[v3.0: carried at Model & Flow v1.4's head.]** To the Board: this section's rulings into the locked ledger. To Matthew, standing open, not blocking: the Emerson contracts ruling as it applies under the new shape, the blind key for the 29 July video (unspent, still owed before any output on that fixture exists), reported-holder junction rows (voicer-only stands until he says otherwise).
...
**[v2.7 note, replacing the v2.6 note: §P8 re-shapes the pass around question-answer pairs and retires abstention rows entirely. Where this section and §P8 disagree on the pass, §P8 wins. What binds §P8's schemas unchanged: one entry per handed unit in order never omitted (now enforced as one-for-one-for-one arithmetic between pair rows, not as silence rows), runtime-closed enums, no identity in free text, and the parse law.]**
...
**[v2.7: SUPERSEDED for the pass — no abstention value is stored at all, per §P8.0's no-abstention-rows ruling; the v2.6 replacement vocabulary (`not_surfaced`, `not_in_excerpts`) was retired unbuilt the same day it was ruled. Completeness is one-for-one-for-one arithmetic between pair rows; a pair the model never disposed throws as our bug, which is the `not_returned` and parse-law protection surviving as a check instead of a row.]**
...
**The riskiest unvalidated assumption in this design, named:** ledger completeness has no comparator on the discovery half. What to measure first: Gate-1-style staged-truth runs per charter, transcripts with known planted people and insights, recall measured against the plant. Until those runs exist, discovery recall is stated only as by-construction, never as a number. **[v2.6: the recall gate at §P8.0 is the first such run, on the 24 June fixture with adjudicated truth.]**
...
**[v2.6: superseded as the pass's asking shape by §P8 — questions once per transcript, the claim path's handed unit is the contract, and the person-crossed-with-dimension grain survives as the Answer ledger's grain, derived by Apex, never enumerated in a prompt. Retained below as the record of the shape that was measured and retired.]**
...
**[v2.6: `_1` is RETIRED from every declared claim set — it stays in this table byte-exact because the ontology is the vendor's and we do not edit history.] [v2.7: the ENTIRE nine-question set is retired from extraction, §P8.4 — Support is read as sentiment into five states and accumulated by the re-promoted counter; these questions remain the wizard's content and the vendor's asset, quizzed on by no model.]**
...
**The counter is demoted, not deleted.** The clamped integer from minus three to plus three, one increment per source-event, remains the **inference fallback for any dimension that has no ontology.** After this recovery that is none of the four, but a bare org, a customer-invented dimension, or a future charter may need it, and it is a ruled and built mechanism that should not be thrown away. **[v2.7: this sentence earned its keep — the counter is RE-PROMOTED to Support's mechanism at §P8.4, Matthew's ruling that sentiment is precisely the dimension where extraction ontology is not feasible; the tree-owns-the-rung ruling is scoped to the fact dimensions and no longer governs Support.]**
...
**[v2.6: criteria placement superseded — decision criteria join People per the v2.5 ruling, contracts at §P8.4; the criteria material below stands as the dedup, citation, and boundary law it always was, and the insight half is untouched.]**
...
**[v2.6: the model-read legs of this ladder (the Test 2 scope read and Test 3's content match) execute inside call 1 of the pass, per §P8.1; the deterministic tests, the two-key lock, and every rule below are unchanged and stay in Apex.]** **[v3.1: the placement half of this bracket is superseded. The model legs execute in CALL 0, before any family read dispatches, because the scope verdict now decides which grains' reads dispatch at all. The deterministic tests, the two-key lock, and every rule below stay in Apex, unchanged. The section on call 0 below carries the ruling.]**
...
**The two-sided summary · added in v0.2, Matthew's.** On a dual-scope call the scope read produces **two summaries from one pass**: the account-focused summary (maintenance, adoption, consumption of what is owned) and the opportunity-focused summary (the expansion, its future close, its decision talk), each side carrying the parts of the call the ontology assigns to it. Each summary then drives its own scope's reads, so the account rubrics see account content and the deal rubrics see deal content, from one transcript, with nothing read twice. **[v3.1: SUPERSEDED as mechanism, Matthew's ruling, 5 August. Two things moved under this ruling since v0.2: the pairs law requires every quote to byte-locate in the frozen artifact, and a summary-fed read cannot emit byte-locating quotes without machinery never built; and the s1 recall finding says variance is whether a read attends to a region, so a pre-filter that mis-sides a sentence makes the miss unrecoverable, no read ever sees those bytes. The purpose survives structurally: each grain's family reads carry that grain's contract set over the whole artifact, account questions meeting account content with no summary standing between the read and the bytes. The scope read itself lives in call 0 and routes only.]**
...
**Named open, not settled here:** whether scope ultimately resolves per claim rather than per call. **[v3.1: CLOSED. Scope resolves per family read's declared grain, finer than per call, coarser than per claim, and deterministic once call 0 declares the grains. The section on call 0 below carries the ruling.]** *(The net-new-person question is closed above in v0.4; the which-opportunity mechanism closed in v0.3 by the B&V test.)*
...
## Call 0 and the grain of a pair · RULED by Matthew, 5 August 2026 · charters v3.1

Three rulings, argued in session with options and costs before any text was written. Together they are the resolver's execution charter; the ladder above is unchanged and executes inside this shape.

**RULING 1 · A PAIR'S SCOPE IS ITS READ'S DECLARED GRAIN.** The resolver declares which grains dispatch: opportunity, account, or both. Each family read goes out at exactly one grain, carrying that grain's contract set and nothing else, whole artifact every time. A pair inherits scope from the read that produced it, and no pair ever carries a model-judged scope. On differently-worded contracts wrong scope is structurally inexpressible, because the other grain's question was never in the read; where the vendor shares wording across grains (the AM_OM support questions), the pair still inherits the read's declared grain by dispatch stamp rather than by wording. This is the failed-twice law applied prospectively: not an instruction to judge scope, a structure where the wrong answer cannot be said. The evidence it rests on: the v1.8 enumeration (no single list is the ontology; the per-map bytes differ down to `organisation` against `organization`), the byte-exact proposition law, and the sweep's attention finding. The stretch-stamp law for an affirmatively named other deal is untouched; it decides which deal, never which grain. Multi-establishment now crosses grains structurally: on a dual call both grains' reads see the whole artifact, and one utterance may pair in both grains' reads, the applicability corollary become structure. The pair's grain stamp needs a field home in the pair ledger; CODE proposes the mechanism first and Model & Flow records it at its bump, the counter's-field pattern. **[v3.2: standing law, one grain dispatching today; the account grain's dispatch, and the grain-stamp proposal, return with the grain.]**

**RULING 2 · CALL 0, ALWAYS.** The resolver's model legs execute as a dedicated read on every Source, before the sweep. Apex runs the deterministic ladder first, unchanged: Test 1 owner's role, the map check, the candidate-set arithmetic, participant overlap. Their outputs travel into call 0 as evidence, which is the ladder's own pattern (the deterministic-but-ambiguous read serves as the interpreter's evidence, not as a verdict). Call 0 reads the whole normalized artifact once and emits, with receipts: the scope verdict (opportunity, account, or dual); **[v3.4: the scope verdict derives from TWO ONE-WAY QUESTIONS answered independently with quoted evidence, opportunity content yes-or-no and account content yes-or-no; DUAL is both yes. The two-sided single question is superseded by the failed-twice law after two majority collapses on the Emerson fixture.]** the opportunity candidate or NONE or AMBIGUOUS, windowed on the evidence-occurred clock; and the people candidate roster including load-bearing mentioned persons. Code dispatches each declared grain's family reads from the verdict. Resolution consolidates here: family reads are pure pair emitters, and the prohibition law takes its clean form, call 0 routes and resolves and never establishes, family reads establish and never route. The conditional variant, skipping call 0 where the deterministic ladder fully resolves, is recorded as a later measured optimization that preserves the law (the verdict exists before dispatch) and is never a design fork; it was not adopted now because Test 1 needs an owner-organizer with a mapped role, and file-dump orgs, roughly 75% of the market by this section's own measure, may carry neither. Call 0's version stamp composes per the two-halves law: the resolver charter prose plus the sided ontology text it reads against, both covered or the stamp is decoration. **[v3.2: unchanged and live. ACCOUNT and DUAL verdicts route and log only, per the backburner; only opportunity-grain family reads dispatch.]** **[v3.3 note, Matthew's, recorded: resolution in call 0 covers the internal side too. Domain recognition classifies internal against external, deterministic; the role map, the same admin-ratified artifact Test 1 reads, classifies internal people seller against service; and call 0 carries the resolved IDs forward, account, opportunity, Contact, and ContactMapDetail rows in the candidate roster, so no downstream stage re-resolves identity. The domain-list fix in Addendum 17 is the permanent deterministic input to this, never a throwaway.]**

**RULING 3 · ACCOUNT-GRAIN ESTABLISHMENTS WRITE NOW.** Matthew's ruling, against design's recommendation, both recorded because the record is the point. Design recommended stamp-and-park: verified account-grain pairs accrue in the ledger with receipts and project nothing until the account-level fold, on the argument that the account grain has zero gradings and the account map is the durable surface this file names as the pollution hazard. Matthew ruled write now: account-grain establishments project per the ruled dual-write machinery, the v0.4 net-new-both-maps rule, per-scope establishment gates, the ruled writer with its mandatory create leg, all live at the account grain. What the ruling requires, named: the account grain's first graded output becomes a gate before any pilot, precision before coverage holding per grain; and the seller-side flag is now BLOCKING before the first account-grain write run, because the sandbox's internal-domain list misses `altify.com` and both Altify participants read buyer side, which under live account writes would land sellers on a customer's account map. **[v3.2: SUPERSEDED same day, before any build. The account grain is backburnered whole: no extraction, no writes, no account contracts. Matthew's ruling, scale named as the driver, readiness recorded as why it costs nothing now. The park recommendation and the write-now ruling both stand above as the record. The seller-side flag reverts to a standing non-blocking defect.]**

**The account-grain contract set · owed, with a recovery item.** The Political set at account grain is authored from the recovered enumeration: the four shared AM_OM questions stand as recovered; the AM versions of the three opportunity-only questions (the counterparts of OM_1, OM_2, OM_9) are owed from the org's own label enumeration before authoring, evidence over inference, never copied from the OM texts, because the vendor's per-map bytes differ. The one-way-half split applies to the AM texts after recovery, same law as the opportunity grain. Sentiment's AM_OM questions are shared wording; the counter moves per map row, so a dual call's sentiment establishments move the row their read's grain names. No insight family is declared at any grain yet. **[v3.2: deferred with the grain. The AM recovery item is cancelled until the grain returns; nothing is authored.]** **[v3.3, Matthew's statement, recorded: at the account grain the family delta is one out and one in. Buyer Role goes away, Decision Orientation comes in, the rest shared. The byte-level wording recovery above stands for wording only, deferred with the grain. Accounts are backburnered, never forgotten; the fixture and key stay attached.]**

**The first fixture and its gate.** The Emerson 17 June call with Matthew's five account-grain rows and his key (adjudication sheet §8), plus the 24 June B&V call as the account-single specimen. The gate asserts five things: call 0 reads Emerson as DUAL; call 0 reads B&V 24 June as ACCOUNT; the Political-subset row (account-level business-case control) locates, identifies, verifies, and projects at the account grain per the key; the insight-territory rows (global rollout and training, post-sale enablement, enterprise AI strategy) and the parked Decision Orientation reading (the CSM/licensing row) report UNREACHABLE on their own lines, never as misses; and the which-opportunity occurred-time window is NOT ASSERTED on this fixture, because the sandbox seed postdates the call by seven weeks, so the gate asserts scope and never the window. The riskiest unvalidated assumption is named: that one scope read detects DUAL rather than collapsing to the majority scope; the two fixtures with differing expected verdicts are its probe, and no result enters a document as a guess. **[v3.2: the gate is revised with the backburner. Asserted now: Emerson reads DUAL; B&V 24 June reads ACCOUNT; the opportunity regression set, 27 standing assertions, holds unchanged with call 0 in front; zero account-grain reads dispatch and zero account-grain writes exist. The projection assertion and the UNREACHABLE lines are deferred with the grain; the five rows and the key stay attached to this fixture for its return. The occurred-window NOT ASSERTED stands.]**

---

## Where this sits

Before the admission gate's per-charter checks, after Locate finds the account. The account is always findable — every call resolves to an account by participants and domains, D360 match rules supplying that join for file-dump orgs on the slow lane, the deterministic email-domain join staying the hot path. **Scope selection is the resolver's job alone; D360 identity resolution answers who, never which scope.** **[v3.1: this placement is now a call of its own, call 0, per the rulings above.]**
...
Q1, the preference gate, opens the reading and places nobody. **[v2.6: Q1 is RETIRED from the claim path entirely — it no longer opens anything.] [v2.7: the whole table above is SUPERSEDED for Support by §P8.4 — the rung derives from the counter's position, not from question conjunctions; the counter positions are the v0.8 mapping, re-promoted. This table stays as the record of the tree that governed between v1.2 and v2.7.]** ~~Political Status derives the same way from its seven once its table is authored; same session, second table, owed.~~ **SUPERSEDED in v2.1: the table is ruled below, and it does not derive the same way. Political Status is a ceiling, not a ladder.**
...
**[v2.4 · The pass execution described below — one model call over the whole declared set — is SUPERSEDED by §P7.3, as is the per-person split that briefly replaced it. Ingest, dedup, roster, adjudication, the invoker and everything downstream of Claim stand. Read §P7.3 for the pass.]** **[v2.6: and §P7.3 is itself superseded as the shape by §P8. Read §P8 for the pass.]**
...
**[v2.6: SUPERSEDED as the pass shape by §P8, per the v2.5 ruling. What survives of this section, named at §P8: byte verification, the writer, everything downstream of Claim, the blind reader's role and schema, the abstention-writer principle (silence costs zero model tokens), `model_missed` as a reported rate, the epistemics sentence's honesty about bounded residual. The stage structure below (one-read inventory, binding calls) is retired. Retained as history and cost data.]**
