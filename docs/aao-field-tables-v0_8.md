...
| `AAO_SHA256__c` / `AAO_Artifact_SHA256__c` | Text(64) | Case-sensitive | Normalisation | Integrity; idempotent re-delivery; the scope-key composer |
...
| `AAO_Subject_Type__c` | Picklist | Restricted, required | Commit | **The discriminator.** Without it, opportunity-plus-proposition and opportunity-plus-contact-plus-dimension rows are indistinguishable to any query that does not already know what it is hunting |
