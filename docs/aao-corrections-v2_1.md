# AAO Corrections and Change Record, v2.1

**v2.1 · 2 August 2026 · The comprehensive record: everything ruled, built, and proven since the four corporate documents were last stamped. Authoritative over them until each is bumped.**

**Changed in v2.1.** CODE audited v2.0 against the org and found three omissions, all real, all added below in their sections: the rubric-scoped-reader law (v2.0 recorded how the applicable set is not decided, without the rule that decides it), the synthetic marker as the product-versus-test discriminator, and the speaker gate's recorded not-applicable for state-based claims. The test count is stated exactly: 139.
...
**The speaker gate records not-applicable for state-based claims; it never skips.** A record has no utterer, so a speaker requirement asked of a state claim is a question that does not apply, and left alone the gate would downgrade every state claim forever. Ruled instead: for basis State the gate writes, in the same field where transcript claims carry their speaker reasoning, that the requirement did not apply and why — nobody said this, the record says it, and the record is named in the claim's cited rows. This is the one place the family law changes what a gate does rather than what a row looks like, and the distinction that matters is recorded versus skipped: no one reading a state claim later can mistake it for one that cleared a check it never faced.

**Deployed and proven in the sandbox** (altify--aossb2, production read-only unconditionally): eight objects live, trigger laws enforced, 139 tests green. Replay is proven: delete all answers, replay claims in evidence-occurred order, reconstruct every answer exactly, on every demo deal. Async arrival survives adjudication failure. Identical bytes ingested twice produce nothing. A seller's own words land downgraded structurally. Every proposition considered writes a candidate row including nobody-said, so the consideration ledger is complete.
...
**The rubric-scoped-reader law, which is how the applicable set IS decided.** One class is the only reader of contracts, and the product path reads exactly the contracts that are active, non-superseded, and unmarked. An unscoped read is a law violation, not a style choice, because its failure mode looks like rigor: after discovery runs, an unscoped model pass sends every rubric's propositions to every deal, and every number moves in the direction that reads as thoroughness — more propositions considered, more reds, more abstentions logged. When upstream question text changes, the old contract generation is superseded, points at its replacement, and keeps its words verbatim forever; supersede is the only retirement, and old claims still name what they were judged under.

**The synthetic marker is the product-versus-test discriminator.** Every AAO object carries a synthetic checkbox written only by seeders. The product path never sees marked rows: they never stream to memory, never project, never count in a roll-up, and the default contract read excludes them. Test and demo entries opt into the synthetic set explicitly, by name, at the call site. The default is the safe one: a new reader that has never heard of any of this gets the product's questions and never the fixture's, so the accident fails toward asking real questions about nothing rather than answering real deals with rehearsal questions.
